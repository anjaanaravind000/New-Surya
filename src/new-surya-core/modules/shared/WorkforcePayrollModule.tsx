...
// ─── Types ────────────────────────────────────────────────────────────────────
type Branch = 'SECONDARY_OUTLET' | 'New Surya' | 'PRIMARY_OUTLET' | 'Wholesale';
...
// ─── Constants ────────────────────────────────────────────────────────────────
const BRANCHES: Branch[] = ['SECONDARY_OUTLET', 'New Surya', 'PRIMARY_OUTLET', 'Wholesale'];
const BRANCH_COLORS: Record<Branch, string> = {
  SECONDARY_OUTLET: 'bg-blue-100 text-blue-800 border-blue-200',
  'New Surya': 'bg-orange-100 text-orange-800 border-orange-200',
  PRIMARY_OUTLET: 'bg-amber-100 text-amber-700 border-amber-200',
  Wholesale: 'bg-emerald-100 text-emerald-800 border-emerald-200',
};
...
function exportExcel(
  ...
  const ws1 = XLSX.utils.aoa_to_sheet([
    ...
    [`New Surya Group | Generated on ${new Date().toLocaleDateString('en-IN')} | Confidential`],
    [],
    salaryHeaders,
    ...salaryRows,
    totals,
  ...
  // Save
  XLSX.writeFile(wb, `NewSurya_SalaryReport_${monthLabel.replace(' ', '_')}.xlsx`);
}
...
function exportAttendanceExcel(
  ...
  const ws = XLSX.utils.aoa_to_sheet([
    [`ATTENDANCE REGISTER — ${monthLabel.toUpperCase()}`],
    [`New Surya Group | Generated on ${new Date().toLocaleDateString('en-IN')} | ${employees.length} employees`],
  ...
  XLSX.writeFile(wb, `NewSurya_Attendance_${monthLabel.replace(' ', '_')}.xlsx`);
}
...
function exportAdvanceExcel(
  ...
  XLSX.writeFile(wb, `NewSurya_Advances_${monthLabel.replace(' ', '_')}.xlsx`);
}
...
function AddEmpModal({ onAdd, onClose }: { onAdd: (e: Employee) => void; onClose: () => void }) {
  ...
  const [branch, setBranch] = useState<Branch>('SECONDARY_OUTLET');
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center p-3" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-card border border-border rounded-2xl w-full max-w-sm max-h-[92vh] overflow-y-auto overscroll-contain">
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button disabled={!valid || saving} onClick={handleAdd} className="flex-1 h-11 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-semibold disabled:opacity-40 active:scale-95 transition-all flex items-center justify-center gap-2">
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-sm max-h-[92vh] overflow-y-auto overscroll-contain" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 pb-5 space-y-2">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} className="flex-1 h-11 rounded-xl border border-border text-sm font-body font-semibold hover:bg-muted transition-colors">Cancel</button>
            <button disabled={!valid || saving} onClick={handleSave} className="flex-1 h-11 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-semibold disabled:opacity-40 active:scale-95 transition-all flex items-center justify-center gap-2">
...
function AnalyticsTab({ employees, att, decisions, daysInMonth, monthLabel, getDecision }: {
  ...
  const branchSalaryData = useMemo(() => {
    return BRANCHES.map(branch => {
      ...
      return { branch: branch === 'New Surya' ? 'Retail' : branch, gross, earned, net, count: emps.length };
    }).filter(d => d.count > 0);
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        ...
        <div className="px-4 py-4 space-y-3">
          ...
          <button
            ...
            onClick={handleAdd}
            className="w-full h-11 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-semibold disabled:opacity-40 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
    ...
    <div className="dashboard-screen min-h-[100dvh] bg-transparent pt-0 pb-6">
            ...
            <div className="absolute top-full right-0 mt-1 bg-card border border-border rounded-xl shadow-xl z-30 min-w-[160px] overflow-hidden">
              {(['All', ...BRANCHES] as const).map(b => (
                <button key={b} onClick={() => { setBranch(b); setShowBranchDD(false); }}
                  className={cn('w-full px-4 py-2.5 text-left text-sm font-body font-semibold transition-colors', b === branch ? 'surya-gradient text-primary-foreground' : 'hover:bg-muted text-foreground')}>
                  {b}
                </button>
              ))}
            </div>
        ...
        <div className="flex items-center bg-card border border-border rounded-xl overflow-hidden">
          {TWO_MONTHS.map((m, i) => (
            <button key={`${m.year}-${m.month}`} onClick={() => setMonthIdx(i)}
              className={cn('flex-1 py-2.5 text-sm font-body font-bold transition-all flex items-center justify-center gap-1.5', i === monthIdx ? 'surya-gradient text-primary-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}>
      ...
      <div className="flex gap-1.5 px-4 mb-3 overflow-x-auto">
        {([
          ...
          <button key={k} onClick={() => handleTabChange(k)}
            className={cn('shrink-0 px-3 py-2.5 rounded-xl text-xs font-body font-bold transition-all active:scale-95', tab === k ? 'surya-gradient text-primary-foreground shadow-sm' : 'bg-card border border-border text-foreground')}>
            {l}
          </button>
        ))}
      </div>
          ...
          {DEPT_OPTIONS.map(d => (
            <button
              ...
              className={cn(
                ...
                dept === d
                  ? 'surya-gradient text-primary-foreground border-transparent shadow-sm'
      ...
      {tab !== 'analytics' && (
        <div className="px-4 mb-3 flex gap-2">
          ...
          {tab === 'employees' && (
            <button onClick={() => setShowAddModal(true)} className="shrink-0 h-10 px-3 rounded-xl surya-gradient text-primary-foreground flex items-center gap-1 text-xs font-body font-semibold active:scale-95 transition-all">
