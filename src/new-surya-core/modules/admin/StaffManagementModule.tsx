...
const ROLE_GROUPS = [
  ...
  },
  {
    ...
    roles: ['stock_audit_secondary', 'stock_audit_primary', 'store', 'baker', 'sweet_master', 'savouries_master', 'cookies_master', 'puffs_master', 'bakery_master', 'cake_master', 'packing'] as UserRole[],
  },
  {
    label: '🏬 Branch Sales',
    roles: ['branch_secondary', 'branch_primary', 'branch_wholesale'] as UserRole[],
  },
  {
    label: '🛡️ Outlet Management & Executive',
    roles: ['branch_incharge_secondary', 'branch_incharge_primary', 'executive'] as UserRole[],
  },
];
...
const ROLE_LABELS: Record<UserRole, string> = {
  ...
  stock_audit_secondary:  'SECONDARY_OUTLET Receiver',
  stock_audit_primary:    'PRIMARY_OUTLET Receiver',
  store:           'Store',
  baker:           'Baker',
  ...
  packing:         'Packing',
  branch_secondary:    'Secondary Retail Outlet',
  branch_primary:      'Retail Outlet',
  branch_wholesale:    'Wholesale Outlet',
  branch_incharge_secondary:     'Secondary Outlet Management',
  branch_incharge_primary:       'Outlet Management',
  executive:           'Executive',
};
...
const ROLE_COLORS: Record<UserRole, string> = {
  ...
  stock_audit_secondary:  'bg-blue-100 text-blue-800',
  stock_audit_primary:    'bg-amber-100 text-amber-800',
  ...
  branch_secondary:    'bg-blue-100 text-blue-800',
  branch_primary:      'bg-amber-100 text-amber-700',
  branch_wholesale:    'bg-emerald-100 text-emerald-800',
  branch_incharge_secondary:     'bg-violet-100 text-violet-800',
  branch_incharge_primary:       'bg-yellow-100 text-yellow-800',
...
export default function StaffManagement() {
  ...
  useEffect(() => { if (searchParams.get('action') === 'add-employee') setShowAdd(true); }, [searchParams]);
    ...
    <div className="dashboard-screen min-h-screen bg-transparent pt-0 pb-6">
      ...
      <div className="px-4 py-4 space-y-3">
          ...
          <div className="bg-card border border-border rounded-2xl p-4 space-y-3 shadow-soft animate-fade-up">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-lg surya-gradient text-primary-foreground text-sm font-body font-bold active:scale-95"
              >
                Add Staff
              </button>
              <button
        ...
        {staffList.map((user) => {
          ...
          const protectedRole = user.role === 'executive' || user.role === 'admin';
          const canManage = actorRole === 'executive' || (!protectedRole && actorRole === 'admin');
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="size-11 rounded-2xl flex items-center justify-center shrink-0"
                      style={{
                        background: ['admin','branch_incharge_secondary','branch_incharge_primary','executive'].includes(user.role) ? 'rgba(245,158,11,0.12)' : 'hsl(var(--muted))',
                        border:     ['admin','branch_incharge_secondary','branch_incharge_primary','executive'].includes(user.role) ? '1px solid rgba(245,158,11,0.25)' : '1px solid hsl(var(--border))',
                      }}>
                      {['admin','branch_incharge_secondary','branch_incharge_primary','executive'].includes(user.role)
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-2 rounded-lg surya-gradient text-primary-foreground text-xs font-body font-bold active:scale-95"
                        >
                          Save
                        </button>
                        <button
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-lg surya-gradient text-primary-foreground text-sm font-body font-bold active:scale-95"
                        >
                          Save
                        </button>
                        <button
