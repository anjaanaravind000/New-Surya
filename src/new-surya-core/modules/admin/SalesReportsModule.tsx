...
export default function SalesReport() {
  ...
  const handleExportExcel = async () => {
    ...
    XLSX.writeFile(wb, `NewSurya_Report_${dateLabel}.xlsx`);
  };
    ...
    <div className="dashboard-screen min-h-screen bg-transparent pt-0 pb-6">
      ...
      <div className="px-4 py-4 space-y-4">
            ...
            <div className="space-y-2">
              {itemSales.map((item, i) => (
                <div key={item.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-body font-semibold text-foreground truncate">{item.name}</p>
                    <div className="w-full bg-muted rounded-full h-1.5 mt-1"><div className="h-full rounded-full surya-gradient" style={{ width: `${(item.qty / (itemSales[0]?.qty || 1)) * 100}%` }} /></div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-body font-bold tabular-nums">{item.qty}</p>
                    <p className="text-[10px] font-body text-muted-foreground tabular-nums">{formatCurrency(item.revenue)}</p>
        ...
        <button
          ...
          className="w-full py-3.5 rounded-xl surya-gradient text-primary-foreground font-body font-bold text-base flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-lg disabled:opacity-40 disabled:pointer-events-none"
        >
