// src/pages/ExecutiveControlModule.tsx
// Executive Dashboard — Sales · Attendance · Credit · Waste Logs
// New in this version:
...
//   • Credit tab: credit age / overdue flagging already exists in ExecutiveCreditTab
...
import ExecutiveCreditTab from '@/components/admin/ExecutiveCreditTab';
import {
...
const EXECUTIVE_FULL_BRANCHES: Branch[] = ['Retail', 'PRIMARY_OUTLET', 'SECONDARY_OUTLET', 'Wholesale'];
const EXECUTIVE_OPERATING_UNITS = ['Retail', 'Retail Outlet', 'Secondary Retail Outlet', 'Wholesale Outlet', 'Bakery Production', 'Store', 'Packing / Dispatch'] as const;

type ExecutiveDatePreset = 'today' | 'yesterday' | '7d' | '15d' | '30d' | 'month';

type ExecutiveAlertTone = 'danger' | 'warning' | 'neutral' | 'success';
...
type ExecutiveStorePurchaseLine = {
  id: string;
  supplierName: string;
...
function executiveDateInput(d = new Date()) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

function executivePresetStart(preset: ExecutiveDatePreset) {
  const d = new Date();
...
function executiveEndOfToday() {
  const d = new Date();
  d.setHours(23, 59, 59, 999);
  return d;
}

function executiveInRange(iso: string | null | undefined, from: Date, to: Date) {
  if (!iso) return false;
  const t = new Date(iso).getTime();
  return Number.isFinite(t) && t >= from.getTime() && t <= to.getTime();
}

function executiveLocalDay(iso: string | null | undefined) {
  if (!iso) return '';
  return executiveDateInput(new Date(iso));
}

function executiveFmtDate(iso: string | null | undefined) {
  if (!iso) return '—';
...
function executiveFmtDateTime(iso: string | null | undefined) {
...
function executiveCsvDownload(filename: string, rows: Array<Record<string, string | number>>) {
...
function executivePrintSection(title: string, html: string) {
  ...
  win.document.write(`
    ...
    </head><body><main><div class="hero"><div><div class="stamp">Executive Report</div><h1>${title}</h1></div><p class="muted">Generated ${new Date().toLocaleString('en-IN')}</p></div>${html}</main></body></html>
  `);
  win.document.close();
  win.print();
}

function executiveBranchDisplay(branch: Branch | string) {
...
function ExecutiveMetricCard({ icon, label, value, sub, tone = 'slate' }: { icon: React.ReactNode; label: string; value: React.ReactNode; sub?: React.ReactNode; tone?: 'green' | 'amber' | 'red' | 'blue' | 'purple' | 'slate' }) {
  return (
    <div className={cn('executive-metric-card', `tone-${tone}`)}>
...
function ExecutiveToolbar({ children }: { children: React.ReactNode }) {
  return <div className="executive-toolbar"><Filter className="size-4" />{children}</div>;
}
...
const EXECUTIVE_BRANCHES = ['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'] as const;
...
function EmptyExecutiveState({ title, message }: { title: string; message: string }) {
  return (
...
function SalesOverviewTab() {
  ...
  useEffect(() => { (['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'] as const).forEach(b => fetchBranchData(b)); }, [fetchBranchData]);
  ...
  const cutoffEnd = useMemo(() => {
    ...
    return executiveEndOfToday();
  }, [dateRange]);
  const salesLedger = useBranchLedger(executiveDateInput(cutoff), executiveDateInput(cutoffEnd), ['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale']);

  const todayStart = useMemo(() => { const d = new Date(); d.setHours(0, 0, 0, 0); return d; }, []);

  const retailOrders = useMemo(() =>
    orders.filter(o => new Date(o.createdAt) >= cutoff && new Date(o.createdAt) <= cutoffEnd && o.status === 'served' && o.paymentType !== 'advance'), // FIX: exclude advance orders (partial totals would distort retailRevenue)
  ...
  const retailRevenue = retailOrders.reduce((s, o) => s + o.total, 0);
  const retailCount   = retailOrders.length;
  const avgOrderValue = retailCount > 0 ? Math.round(retailRevenue / retailCount) : 0;
  ...
  const branchSales = useMemo(() => {
    const branches: Record<string, { qty: number; count: number; revenue: number }> = {
      SECONDARY_OUTLET: { qty: 0, count: 0, revenue: 0 },
      PRIMARY_OUTLET:   { qty: 0, count: 0, revenue: 0 },
      Wholesale: { qty: 0, count: 0, revenue: 0 },
    };
    (['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'] as const).forEach(b => {
  ...
  const grandTotal         = retailRevenue + totalBakeryRevenue;
  const selectedBakeryRevenue = branchFilter === 'all'
    ? totalBakeryRevenue
    : branchFilter === 'Retail' ? 0 : branchSales[branchFilter].revenue;
  const selectedBakeryQty = branchFilter === 'all'
    ? totalBakeryQty
    : branchFilter === 'Retail' ? 0 : branchSales[branchFilter].qty;
  const selectedRetailRevenue = branchFilter === 'all' || branchFilter === 'Retail' ? retailRevenue : 0;
  const selectedGrandTotal = selectedRetailRevenue + selectedBakeryRevenue;
  ...
  const dailyRevenueData = useMemo(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const retailRev = retailOrders.filter(o => new Date(o.createdAt).toDateString() === dateStr)
        .reduce((s, o) => s + o.total, 0);
      const secondary_outletRev = (sales['SECONDARY_OUTLET'] || []).filter(s => s.soldAt && new Date(s.soldAt).toDateString() === dateStr)
        .reduce((s, sale) => s + ((sale as typeof sale & { unitPrice?: number }).unitPrice || 0) * sale.quantitySold, 0);
      const primary_outletRev = (sales['PRIMARY_OUTLET'] || []).filter(s => s.soldAt && new Date(s.soldAt).toDateString() === dateStr)
        .reduce((s, sale) => s + ((sale as typeof sale & { unitPrice?: number }).unitPrice || 0) * sale.quantitySold, 0);
      // Wholesale wholesale shop-supply revenue (wholesale_bills) is NOT included — it lives in a separate
      // Wholesale wholesale bills are mirrored into branch_sales after confirmation.
      const wholesaleRev = (sales['Wholesale'] || []).filter(s => s.soldAt && new Date(s.soldAt).toDateString() === dateStr)
        .reduce((s, sale) => s + ((sale as typeof sale & { unitPrice?: number }).unitPrice || 0) * sale.quantitySold, 0);
      const label = d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' });
      result.push({ date: label, Retail: retailRev, SECONDARY_OUTLET: secondary_outletRev, PRIMARY_OUTLET: primary_outletRev, Wholesale: wholesaleRev,
        Total: retailRev + secondary_outletRev + primary_outletRev + wholesaleRev });
    }
    return result;
  }, [retailOrders, sales, dateRange]);

  const revShareData = [
    { name: 'Retail',  value: retailRevenue,              color: COLORS[0] },
    { name: 'Secondary Outlet', value: branchSales.SECONDARY_OUTLET.revenue, color: COLORS[2] },
    { name: 'Primary Outlet',   value: branchSales.PRIMARY_OUTLET.revenue,   color: COLORS[1] },
    { name: 'Wholesale Outlet', value: branchSales.Wholesale.revenue,  color: COLORS[4] },
  ].filter(d => d.value > 0);

  // CHANGE 5d: combined all-branch payment breakdown
  const retailPayBreakdown = useMemo(() => {
    let cash = 0, upi = 0, card = 0;
  ...
  const allPayBreakdown = useMemo(() => {
    const includeRetail = branchFilter === 'all' || branchFilter === 'Retail';
    let cash = includeRetail ? retailPayBreakdown.cash : 0;
    let upi = includeRetail ? retailPayBreakdown.upi : 0;
    let card = includeRetail ? retailPayBreakdown.card : 0;
    let credit = 0;
    ...
    return [{ name: 'Cash', value: cash }, { name: 'UPI', value: upi },
      ...
      .filter(p => p.value > 0);
  }, [retailPayBreakdown, bills, cutoff, cutoffEnd, branchFilter]);

  const topRetailItems = useMemo(() => {
    const map = new Map<string, { qty: number; revenue: number }>();
    retailOrders.forEach(o => o.items.forEach(ci => {
  ...
  const peakHours = useMemo(() => {
    ...
    retailOrders.forEach(o => { counts[new Date(o.createdAt).getHours()]++; });
  ...
  const maxBranchRev = Math.max(retailRevenue, branchSales.SECONDARY_OUTLET.revenue, branchSales.PRIMARY_OUTLET.revenue, branchSales.Wholesale.revenue, 1);

  // CHANGE 5e: export
  const exportSales = () => executiveCsvDownload(`Sales_${dateRange}.csv`, [
    ...(branchFilter === 'all' || branchFilter === 'Retail' ? retailOrders : []).map(o => ({
      Branch: 'Retail', Date: executiveFmtDate(o.createdAt),
      Revenue: o.total, Payment: o.paymentType, Items: o.items.length,
    })),
    ...(['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'] as const)
      ...
      (sales[b] || []).filter(s => s.soldAt && new Date(s.soldAt) >= cutoff && new Date(s.soldAt) <= cutoffEnd).map(s => ({
        Branch: b, Date: executiveFmtDate(s.soldAt),
  ...
  const showRetail     = branchFilter === 'all' || branchFilter === 'Retail';
  const showBranches = branchFilter === 'all' || branchFilter !== 'Retail';
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <select value={branchFilter} onChange={e => setBranchFilter(e.target.value as Branch | 'all')}
          ...
          <option value="SECONDARY_OUTLET">Secondary Retail Outlet</option>
          <option value="PRIMARY_OUTLET">Retail Outlet</option>
      ...
      {showRetail && (
        <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-2xl p-4">
          <p className="text-[10px] font-semibold text-primary uppercase tracking-widest mb-1">Today's Collection (Retail)</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {showRetail && <KPI icon={<Store       className="size-4" />} label="Retail Revenue"    value={formatCurrency(retailRevenue)}   sub={`${retailCount} orders`}       color="bg-emerald-50 text-emerald-700" />}
        ...
        {showRetail && <KPI icon={<TrendingUp className="size-4" />} label="Avg Order Value" value={formatCurrency(avgOrderValue)} sub="Retail only" color="bg-blue-50 text-blue-700" />}
      </div>
      ...
      {showBranches && (
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className="space-y-3">
            {[
              { label: 'Retail',  value: retailRevenue,              color: 'bg-emerald-500', qty: `${retailCount} orders`, show: showRetail },
              { label: 'Secondary Outlet', value: branchSales.SECONDARY_OUTLET.revenue, color: 'bg-blue-500',   qty: `${branchSales.SECONDARY_OUTLET.qty} items`, show: branchFilter === 'all' || branchFilter === 'SECONDARY_OUTLET' },
              { label: 'Primary Outlet',   value: branchSales.PRIMARY_OUTLET.revenue,   color: 'bg-amber-500',  qty: `${branchSales.PRIMARY_OUTLET.qty} items`, show: branchFilter === 'all' || branchFilter === 'PRIMARY_OUTLET' },
              { label: 'Wholesale Outlet', value: branchSales.Wholesale.revenue,  color: 'bg-purple-500', qty: `${branchSales.Wholesale.qty} items`, show: branchFilter === 'all' || branchFilter === 'Wholesale' },
          ...
          <ResponsiveContainer width="100%" height={200}>
            {branchFilter === 'all' ? (
              <LineChart data={dailyRevenueData}>
                ...
                <Tooltip formatter={(v: number) => [formatCurrency(v)]} />
                <Legend />
                <Line type="monotone" dataKey="Retail"  stroke={COLORS[0]} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="SECONDARY_OUTLET" stroke={COLORS[2]} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="PRIMARY_OUTLET"   stroke={COLORS[1]} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="Wholesale" stroke={COLORS[4]} strokeWidth={2} dot={false} />
              </LineChart>
            ) : (
              <AreaChart data={dailyRevenueData}>
                ...
                <Area type="monotone" dataKey={branchFilter === 'Retail' ? 'Retail' : branchFilter}
      ...
      {allPayBreakdown.length > 0 && (
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="font-display text-base font-bold mb-3">Payment Methods — {branchFilter === 'all' ? 'All Branches' : branchFilter === 'Retail' ? 'Retail' : `${branchFilter} Branch`}</h3>
      ...
      {/* Peak Hours */}
      {retailOrders.length > 0 && showRetail && (
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="font-display text-base font-bold mb-1 flex items-center gap-2">
            <Clock className="size-4 text-primary" />Peak Hours (Retail)
          </h3>
      ...
      {/* Top Items */}
      {topRetailItems.length > 0 && showRetail && (
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="font-display text-base font-bold mb-4">Top Retail Items by Revenue</h3>
          <ResponsiveContainer width="100%" height={Math.max(topRetailItems.length * 32, 180)}>
            <BarChart data={topRetailItems} layout="vertical">
...
type ExecutiveAttendanceRow = { employee_id: string; day: number; present: boolean; half: boolean; woff: boolean; bf: boolean; lunch: boolean; dinner: boolean };
type ExecutiveMonthAttendance = Record<string, ExecutiveAttendanceRow>;
type ExecutiveDeductionDecision = { deductAdvance: boolean; deductOther: boolean; deductUniform: boolean; deductESI: boolean; deductPF: boolean };
type ExecutiveDeductionDecisions = Record<string, ExecutiveDeductionDecision>;
const executiveDefaultDecision = (): ExecutiveDeductionDecision => ({ deductAdvance: false, deductOther: true, deductUniform: true, deductESI: false, deductPF: false });
const EXECUTIVE_ESI_RATE = 0.0075;
const EXECUTIVE_PF_RATE = 0.12;
const EXECUTIVE_ESI_WAGE_LIMIT = 21000;
const executiveAttendanceKey = (employeeId: string, day: number) => `${employeeId}-${day}`;
const executiveCalcESI = (earned: number, gross: number) => gross <= EXECUTIVE_ESI_WAGE_LIMIT ? Math.round(earned * EXECUTIVE_ESI_RATE) : 0;
const executiveCalcPF = (earned: number) => Math.min(1800, Math.round(earned * EXECUTIVE_PF_RATE));

function executiveCalcSalary(emp: { id: string; grossSalary: number; salaryAdvance: number; uniformDeduction: number; otherDeduction: number }, attendance: ExecutiveMonthAttendance, daysInMonth: number, decision: ExecutiveDeductionDecision = executiveDefaultDecision()) {
  let presentDays = 0;
  let halfDays = 0;
  let woffDays = 0;
  let canteenTotal = 0;
  for (let day = 1; day <= daysInMonth; day += 1) {
    const row = attendance[executiveAttendanceKey(emp.id, day)];
    if (!row) continue;
  ...
  const deductions = (decision.deductAdvance ? Number(emp.salaryAdvance || 0) : 0)
    ...
    + canteenTotal
    + (decision.deductESI ? executiveCalcESI(earned, Number(emp.grossSalary || 0)) : 0)
    + (decision.deductPF ? executiveCalcPF(earned) : 0);
...
function AttendanceSalaryTab() {
  ...
  const [attendance, setAttendance] = useState<ExecutiveMonthAttendance>({});
  const [deductionDecisions, setDeductionDecisions] = useState<ExecutiveDeductionDecisions>({});
  ...
  const loadEmployees = useCallback(() => {
    ...
    Promise.all([
      ...
      if (error) setFetchError(error.message || 'Failed to load employee data.');
        ...
        const nextAttendance: ExecutiveMonthAttendance = {};
        (attendanceRes.data ?? []).forEach((row: any) => {
          nextAttendance[executiveAttendanceKey(row.employee_id, Number(row.day))] = {
        ...
        const nextDecisions: ExecutiveDeductionDecisions = {};
  ...
  const salaryStats = useMemo(() => {
    ...
    const earned     = employees.reduce((a, e) => a + executiveCalcSalary(e, attendance, daysInMonth, deductionDecisions[e.id] ?? executiveDefaultDecision()).earned, 0);
    const advances   = employees.reduce((a, e) => a + (Number(e.salaryAdvance) || 0), 0);
    const deductions = employees.reduce((a, e) => a + executiveCalcSalary(e, attendance, daysInMonth, deductionDecisions[e.id] ?? executiveDefaultDecision()).deductions, 0);
    const netPayable = employees.reduce((a, e) => a + executiveCalcSalary(e, attendance, daysInMonth, deductionDecisions[e.id] ?? executiveDefaultDecision()).net, 0);
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        ...
        {Object.entries(branchGroups).map(([branch, emps]) => {
          const branchTotal = emps.reduce((s, e) => s + executiveCalcSalary(e, attendance, daysInMonth, deductionDecisions[e.id] ?? executiveDefaultDecision()).earned, 0);
...
function WasteLogsTab() {
  // Kitchen waste log (retail)
  const [entries, setEntries] = useState<Array<{
    id: string; food_item: string; quantity: string; logged_at: string;
  }>>([]);
  // Branch waste logs — shared with Ordering & Receiving / Outlet Management / Secondary Outlet Management via
  ...
  const BRANCH_COLORS_MAP: Record<string, string> = {
    PRIMARY_OUTLET: 'bg-blue-50 text-blue-700',
    SECONDARY_OUTLET: 'bg-purple-50 text-purple-700',
        ...
        <>
          ...
          {filteredBranchWaste.length > 0 && (
            <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
              {(['PRIMARY_OUTLET', 'SECONDARY_OUTLET', 'Wholesale', 'Retail'] as const).map(branch => {
...
// ── Executive Audit Logs Tab (read-only) ─────────────────────────────────────────
function ExecutiveAuditTab() {
  const { auditLogs } = useBranchOpsStore();
  const { fetchBranchData } = useBranchStore();
  const [search, setSearch] = useState('');
  const [branchFilter, setBranchFilter] = useState<string>('all');

  useEffect(() => { EXECUTIVE_FULL_BRANCHES.forEach(b => fetchBranchData(b)); }, [fetchBranchData]);
  ...
  const BRANCH_COLOR: Record<string, string> = { PRIMARY_OUTLET: 'bg-blue-50 text-blue-700', SECONDARY_OUTLET: 'bg-purple-50 text-purple-700', Wholesale: 'bg-amber-50 text-amber-700', Retail: 'bg-emerald-50 text-emerald-700' };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select value={branchFilter} onChange={e => setBranchFilter(e.target.value)} className="rounded-2xl border border-border bg-card px-3 py-2 text-sm outline-none">
          <option value="all">All branches</option>
          {EXECUTIVE_FULL_BRANCHES.map(b => <option key={b} value={b}>{executiveBranchDisplay(b)}</option>)}
        </select>
      </div>
...
// ── Executive Complaints Tab ──────────────────────────────────────────────────────
function ExecutiveComplaintsTab() {
  ...
  const BRANCH_COLOR: Record<string, string> = {
    PRIMARY_OUTLET: 'bg-blue-50 text-blue-700 border-blue-200',
    SECONDARY_OUTLET: 'bg-purple-50 text-purple-700 border-purple-200',
  ...
  const handleUpdate = async (id: string, currentStatus: string) => {
    ...
    setSaving(id);
    updateComplaintStatus(id, newStatus as 'Open' | 'In Review' | 'Resolved', 'Executive');
    setSaving(null);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">All outlet complaints — executive view</p>
        </div>
      </div>
...
function BranchOverviewTab() {
  ...
  const [preset, setPreset] = useState<ExecutiveDatePreset>('today');
  const SALES_UNITS = ['Retail', 'Retail Outlet', 'Secondary Retail Outlet', 'Wholesale Outlet'];
  ...
  // useBranchOpsStore (that array only ever holds Retail/PRIMARY_OUTLET/SECONDARY_OUTLET/Wholesale
  ...
  useEffect(() => { startPolling(60); return () => stopPolling(); }, [startPolling, stopPolling]);
  useEffect(() => { EXECUTIVE_FULL_BRANCHES.forEach(branch => fetchBranchData(branch)); }, [fetchBranchData]);
  useEffect(() => { void fetchStockMismatches(); }, [fetchStockMismatches]);

  const from = useMemo(() => executivePresetStart(preset), [preset]);
  const to = useMemo(() => {
    if (preset === 'yesterday') {
      const d = new Date(); d.setDate(d.getDate() - 1); d.setHours(23, 59, 59, 999); return d;
    }
  ...
  const fromKey = useMemo(() => executiveDateInput(from), [from]);
  const toKey = useMemo(() => executiveDateInput(to), [to]);
  const executiveLedger = useBranchLedger(fromKey, toKey, ['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale']);

  const branchStockAlertCount = useCallback((branch: Branch) => {
    if (branch === 'Wholesale') return 0;

    // Executive alerts must represent real stock mismatch events, not every
  ...
  const branchPurchaseSummary = useCallback((branch: Branch) => {
    const records = purchases.filter(p => p.branch === branch && executiveInRange(p.createdAt, from, to));
  ...
  const branchRows = useMemo(() => EXECUTIVE_OPERATING_UNITS.map((unit) => {
    if (unit === 'Retail') {
      const served = orders.filter(o => o.status === 'served' && executiveInRange(o.createdAt, from, to));
      const cancelled = orders.filter(o => o.status === 'cancelled' && executiveInRange(o.createdAt, from, to));
      ...
      return {
        ...
        stockAlerts: 0, closureStatus: served.length ? 'Review Retail closure' : 'No sale yet',
        keyAlert: cancelled.length ? `${cancelled.length} cancelled orders` : 'Kitchen and billing live',
      };
    }

    if (unit === 'Bakery Production') {
      const pending = storeOrders.filter(o => executiveInRange(o.createdAt, from, to) && !['Delivered', 'Rejected'].includes(o.status)).length;
    ...
    if (unit === 'Store') {
      ...
      const invoicesInRange = storeInvoiceRows.filter(row => executiveInRange(row.date, from, to));
    ...
    if (unit === 'Packing / Dispatch') {
      const pendingDispatch = storeOrders.filter(o => executiveInRange(o.createdAt, from, to) && ['Confirmed', 'Ready'].includes(o.status)).length;
      return {
        unit, sales: 0, netSales: 0, cash: 0, upi: 0, card: 0, credit: 0,
        expenses: 0, purchases: 0, pendingPayments: 0, pendingCredit: 0,
    ...
    const ledgerRows = executiveLedger.closureRows.filter(row => row.branch === branch);
    if (ledgerRows.length > 0) {
      const gross = ledgerRows.reduce(
        (sum, row) => sum + Math.max(
          0,
          executiveLedger.toNumber(row.sales_total)
            - executiveLedger.toNumber(row.advance_collected)
            - executiveLedger.toNumber(row.advance_balance_collected),
        ),
        0,
      );
      const cash = ledgerRows.reduce((sum, row) => sum + executiveLedger.toNumber(row.cash_total), 0);
      const upi = ledgerRows.reduce((sum, row) => sum + executiveLedger.toNumber(row.upi_total), 0);
      const card = ledgerRows.reduce((sum, row) => sum + executiveLedger.toNumber(row.card_total), 0);
      const credit = ledgerRows.reduce((sum, row) => sum + executiveLedger.toNumber(row.credit_billed), 0);
      const savedClosure = executiveLedger.savedClosures.find(c => c.branch === branch);
      const stockAlertCount = branchStockAlertCount(branch);
      const openCredit = (creditSales[branch] || [])
        .filter((sale) => sale.status !== 'settled' && executiveInRange(sale.createdAt, from, to))
      ...
      return {
        ...
        expenses: executiveLedger.toNumber(savedClosure?.expenses || 0), purchases: branchPurchaseInfo.total, pendingPayments: openCredit + branchPurchaseInfo.pending, pendingCredit: openCredit,
        stockAlerts: stockAlertCount + pendingIncomingCount(branch),
        closureStatus: savedClosure ? (Math.abs(executiveLedger.toNumber(savedClosure.difference)) > 0 ? 'Difference in closure' : 'Closed') : 'Pending closure',
        keyAlert: branch === 'Wholesale'
    ...
    const dbSales = (sales[branch] || []).filter(s => executiveInRange(s.soldAt, from, to));
    const localBills = bills.filter(b => b.branch === branch && executiveInRange(b.createdAt, from, to) && b.status !== 'Returned');
    const branchReturns = returns.filter(r => r.branch === branch && executiveInRange(r.createdAt, from, to));
    ...
    const stockAlertCount = branchStockAlertCount(branch);
    const openCredit = (creditSales[branch] || [])
      .filter((credit) => credit.status !== 'settled' && executiveInRange(credit.createdAt, from, to))
      .reduce((sum, credit) => sum + moneyNumber(credit.creditAmount), 0);
    const lastClosure = cashierClosures.find(c => c.branch === branch && executiveInRange(c.createdAt, from, to));
    ...
    return {
      ...
      expenses: cashMovements.filter((movement) => movement.branch === branch && movement.direction === 'out' && !String(movement.purpose || '').toLowerCase().startsWith('purchase payment') && executiveInRange(movement.dateTime, from, to)).reduce((sum, movement) => sum + moneyNumber(movement.amount), 0), purchases: branchPurchaseInfo.total, pendingPayments: openCredit + branchPurchaseInfo.pending, pendingCredit: openCredit,
      stockAlerts: stockAlertCount + pendingIncomingCount(branch),
  ...
  }), [executiveLedger, orders, sales, advanceOrders, creditSales, bills, returns, cashMovements, cashierClosures, storeOrders, storeInvoiceRows, from, to, branchStockAlertCount, pendingIncomingCount, branchPurchaseSummary]);
  ...
  const overallCreditPending = useMemo(() => EXECUTIVE_FULL_BRANCHES.reduce((grandTotal, branch) => (
  ...
  const PRESET_OPTIONS: Array<{ label: string; value: ExecutiveDatePreset }> = [
    { label: 'Today',      value: 'today' },
    { label: 'Yesterday',  value: 'yesterday' },
    { label: '7 Days',     value: '7d' },
    { label: '15 Days',    value: '15d' },
  ...
  return (
    <div className="executive-tab-stack">
      <ExecutiveToolbar>
        ...
        <button type="button" onClick={() => executiveLedger.refresh()} disabled={executiveLedger.loading} className="inline-flex items-center gap-1.5 disabled:opacity-60"><RefreshCw className={cn('size-4', executiveLedger.loading && 'animate-spin')} />Refresh</button>
        <button type="button" onClick={() => executiveCsvDownload('executive-branch-overview.csv', branchRows.map(r => ({ Unit: r.unit, Sales: r.sales, NetSales: r.netSales, Purchases: r.purchases, PendingPayments: r.pendingPayments, Alerts: r.stockAlerts, Closure: r.closureStatus })))}><Download className="size-4" />Export</button>
      </ExecutiveToolbar>

      <section className="executive-metric-grid wide">
        <ExecutiveMetricCard icon={<IndianRupee className="size-5" />} label="Gross Sales" value={formatCurrency(totals.sales)} sub="Retail + branches" tone="green" />
        <ExecutiveMetricCard icon={<TrendingUp className="size-5" />} label="Net Sales" value={formatCurrency(totals.netSales)} sub="After returns" tone="blue" />
        <ExecutiveMetricCard icon={<ShoppingBag className="size-5" />} label="Purchases" value={formatCurrency(storePurchases)} sub="Store only" tone="purple" />
        <ExecutiveMetricCard icon={<WalletCards className="size-5" />} label="Pending Payments" value={formatCurrency(overallCreditPending)} sub="Overall customer credit outstanding" tone="amber" />
      </section>

      {/* CHANGE 4c: Grouped payment bar chart for sales branches */}
      {paymentChartData.length > 0 && (
        <section className="executive-panel">
          <div className="executive-panel-head"><div><span>Payment Breakdown by Branch</span><h3>Cash · UPI · Card · Credit</h3></div></div>
      ...
      <section className="executive-business-grid">
        {visibleRows.map(row => {
          ...
          const badgeClass = row.closureStatus.toLowerCase().includes('pending') ? 'warn'
            ...
            : '';
          return (
            <article key={row.unit} className={cn('executive-business-card', borderClass)}>
              <div className="executive-business-card-head">
...
// ── Daily Closure Tab ────────────────────────────────────────────────────────
function ExecutiveDailyClosureTab() {
  const { orders, startPolling, stopPolling } = useOrderStore();
  const { bills, returns, purchasePayments, bankDeposits, cashierClosures, cashMovements } = useBranchOpsStore();
  const [date, setDate] = useState(executiveDateInput());
  const [branch, setBranch] = useState<'all' | Branch>('all');
  const executiveLedger = useBranchLedger(date, date, ['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale']);

  useEffect(() => { startPolling(60); return () => stopPolling(); }, [startPolling, stopPolling]);

  const rows: ExecutiveClosureRow[] = useMemo(() => EXECUTIVE_FULL_BRANCHES.map(b => {
    if (b === 'Retail') {
      const dayOrders = orders.filter(o => executiveLocalDay(o.createdAt) === date && o.status === 'served');
      ...
      return { branch: executiveBranchDisplay(b), opening: 0, grossSales: gross, returns: 0, netSales: gross, cash, upi, card, credit, expenses: 0, purchases: 0, bankDeposits: 0, expectedCash: cash, countedCash: 0, difference: 0, status: 'Pending' as ExecutiveClosureRow['status'], closedBy: 'Retail cashier', closedAt: '', remarks: 'Retail closure is verified in Daily Closure module.' };
    }
    const ledger = executiveLedger.closureByBranchDate.get(`${b}:${date}`);
    const savedLedgerClosure = executiveLedger.savedClosureByBranchDate.get(`${b}:${date}`);
    if (ledger) {
      const gross = Math.max(
        0,
        executiveLedger.toNumber(ledger.sales_total)
          - executiveLedger.toNumber(ledger.advance_collected)
          - executiveLedger.toNumber(ledger.advance_balance_collected),
      );
      const ret = executiveLedger.toNumber(savedLedgerClosure?.refunds || 0);
      const cash = executiveLedger.toNumber(ledger.cash_total);
      const upi = executiveLedger.toNumber(ledger.upi_total);
      const card = executiveLedger.toNumber(ledger.card_total);
      const credit = executiveLedger.toNumber(ledger.credit_billed);
      const expenses = executiveLedger.toNumber(savedLedgerClosure?.expenses || 0);
      const purchasePayments = executiveLedger.toNumber(savedLedgerClosure?.purchase_payments || 0);
      const bankDeposits = executiveLedger.toNumber(savedLedgerClosure?.bank_deposits || 0);
      const expectedCash = savedLedgerClosure ? executiveLedger.toNumber(savedLedgerClosure.expected_cash) : Math.max(0, cash - expenses - purchasePayments - bankDeposits);
      const countedCash = savedLedgerClosure ? executiveLedger.toNumber(savedLedgerClosure.actual_cash) : 0;
      const difference = savedLedgerClosure ? executiveLedger.toNumber(savedLedgerClosure.difference) : 0;
      return {
        branch: executiveBranchDisplay(b), opening: executiveLedger.toNumber(savedLedgerClosure?.opening_cash || 0), grossSales: gross, returns: ret, netSales: Math.max(0, gross - ret), cash, upi, card, credit,
        expenses, purchases: purchasePayments, bankDeposits, expectedCash, countedCash, difference,
        status: (savedLedgerClosure ? (Math.abs(difference) > 0 ? 'Difference' : 'Completed') : 'Pending') as ExecutiveClosureRow['status'],
    ...
    const dayBills = bills.filter(bill => bill.branch === b && executiveLocalDay(bill.createdAt) === date && bill.status !== 'Returned');
    const dayReturns = returns.filter(ret => ret.branch === b && executiveLocalDay(ret.createdAt) === date);
    const dayPayments = purchasePayments.filter(pay => pay.branch === b && executiveLocalDay(pay.createdAt) === date).reduce((sum, p) => sum + moneyNumber(p.amount), 0);
    const dayDeposits = bankDeposits.filter(dep => dep.branch === b && executiveLocalDay(dep.createdAt) === date).reduce((sum, d) => sum + moneyNumber(d.amount), 0);
    const dayExpenses = cashMovements.filter(m => m.branch === b && executiveLocalDay(m.dateTime) === date && m.direction === 'out' && !String(m.purpose || '').toLowerCase().startsWith('purchase payment')).reduce((sum, m) => sum + moneyNumber(m.amount), 0);
    const closure = cashierClosures.find(c => c.branch === b && executiveLocalDay(c.createdAt) === date);
    ...
    return {
      branch: executiveBranchDisplay(b), opening: closure?.openingCash ?? 0, grossSales: gross, returns: ret, netSales: Math.max(0, gross - ret), cash, upi, card, credit,
      expenses: dayExpenses, purchases: dayPayments, bankDeposits: dayDeposits, expectedCash, countedCash, difference,
      status: (closure ? (Math.abs(difference) > 0 ? 'Difference' : 'Completed') : 'Pending') as ExecutiveClosureRow['status'],
      closedBy: closure?.cashier || 'Pending', closedAt: closure?.createdAt || '', remarks: closure?.notes || (closure ? 'Closed' : 'Closure not submitted'),
    };
  }).filter(row => branch === 'all' || row.branch === executiveBranchDisplay(branch)), [executiveLedger, orders, bills, returns, purchasePayments, bankDeposits, cashierClosures, cashMovements, date, branch]);
  ...
  const print = () => executivePrintSection('Executive Daily Closure Overview', `<table><thead><tr><th>Branch</th><th>Status</th><th>Net sales</th><th>Cash</th><th>Expected cash</th><th>Counted cash</th><th>Difference</th><th>Closed by</th><th>Remarks</th></tr></thead><tbody>${rows.map(r => `<tr><td>${r.branch}</td><td>${r.status}</td><td>${formatCurrency(r.netSales)}</td><td>${formatCurrency(r.cash)}</td><td>${formatCurrency(r.expectedCash)}</td><td>${formatCurrency(r.countedCash)}</td><td>${formatCurrency(r.difference)}</td><td>${r.closedBy}</td><td>${r.remarks}</td></tr>`).join('')}</tbody></table>`);

  return (
    <div className="executive-tab-stack">
      {/* CHANGE 8a: date presets */}
      <ExecutiveToolbar>
        <button type="button" onClick={() => setDate(executiveDateInput())}
          className={cn(date === executiveDateInput() && 'is-active')}>Today</button>
        <button type="button" onClick={() => { const d = new Date(); d.setDate(d.getDate()-1); setDate(executiveDateInput(d)); }}>Yesterday</button>
        <input type="date" value={date} onChange={e => setDate(e.target.value)} />
        <select value={branch} onChange={e => setBranch(e.target.value as 'all' | Branch)}><option value="all">All business units</option>{EXECUTIVE_FULL_BRANCHES.map(b => <option key={b} value={b}>{executiveBranchDisplay(b)}</option>)}</select>
        <button type="button" onClick={print}><Printer className="size-4" />Print</button>
        <button type="button" onClick={() => executiveLedger.refresh()} disabled={executiveLedger.loading} className="inline-flex items-center gap-1.5 disabled:opacity-60"><RefreshCw className={cn('size-4', executiveLedger.loading && 'animate-spin')} />Refresh</button>
        <button type="button" onClick={() => executiveCsvDownload('executive-daily-closure.csv', rows.map(r => ({ Branch: r.branch, Status: r.status, Opening: r.opening, TotalSales: r.grossSales, Cash: r.cash, UPI: r.upi, Card: r.card, Credit: r.credit, Returns: r.returns, NetSales: r.netSales, Expenses: r.expenses, PurchasePayments: r.purchases, BankDeposits: r.bankDeposits, Closing: r.expectedCash, Difference: r.difference, ClosedBy: r.closedBy, ClosedAt: executiveFmtDateTime(r.closedAt), Remarks: r.remarks })))}><FileSpreadsheet className="size-4" />Export</button>
      </ExecutiveToolbar>

      <section className="executive-metric-grid">
        <ExecutiveMetricCard icon={<Receipt className="size-5" />} label="Net Sales" value={formatCurrency(totals.net)} tone="green" />
        <ExecutiveMetricCard icon={<Banknote className="size-5" />} label="Cash Expected" value={formatCurrency(totals.cash)} tone="blue" />
        <ExecutiveMetricCard icon={<AlertTriangle className="size-5" />} label="Difference" value={formatCurrency(totals.diff)} tone={Math.abs(totals.diff) > 0 ? 'red' : 'green'} />
        <ExecutiveMetricCard icon={<Clock className="size-5" />} label="Pending Closures" value={totals.pending} tone={totals.pending ? 'amber' : 'green'} />
      </section>
      ...
      <section className="executive-business-grid">
        {rows.map(row => {
          ...
          return (
            <article key={row.branch} className={cn('executive-business-card closure', borderClass)}>
              <div className="executive-business-card-head">
              ...
              <footer><ShieldCheck className="size-4" />{row.closedBy} · {executiveFmtDateTime(row.closedAt)} · {row.remarks}</footer>
            </article>
          );
        })}
      </section>
...
// ── Executive Alerts Tab ─────────────────────────────────────────────────────────
function ExecutiveAlertsTab() {
  const { orders, startPolling, stopPolling } = useOrderStore();
  ...
  const [tone, setTone] = useState<'all' | ExecutiveAlertTone>('all');
  const [dismissed, setDismissed] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(localStorage.getItem('executive-dismissed-alerts') || '[]') as string[]); } catch { return new Set(); }
  });
  ...
  const alerts: ExecutiveAlert[] = useMemo(() => {
    const today = executiveDateInput();
    const list: ExecutiveAlert[] = [];
    EXECUTIVE_FULL_BRANCHES.forEach(branch => {
      const dbCredits = (creditSales[branch] || []).filter(c => c.status !== 'settled' && c.dueDate && new Date(c.dueDate) < new Date());
      if (dbCredits.length) list.push({ title: 'Overdue branch credit', value: String(dbCredits.length), note: `${executiveBranchDisplay(branch)} credit due follow-up`, tone: 'danger', branch });
      const closedToday = cashierClosures.some(c => c.branch === branch && executiveLocalDay(c.createdAt) === today);
      if (branch !== 'Retail' && !closedToday) list.push({ title: 'Daily closure pending', value: '1', note: `${executiveBranchDisplay(branch)} closure not submitted for today`, tone: 'warning', branch });
    });
    ...
    if (pendingInvoice.length) list.push({ title: 'Store invoices pending review', value: String(pendingInvoice.length), note: 'Executive can review purchase exposure', tone: 'warning' });
    ...
    const todayReturns = returns.filter(r => executiveLocalDay(r.createdAt) === today);
    ...
    const cancelled = orders.filter(o => executiveLocalDay(o.createdAt) === today && o.status === 'cancelled');
    if (cancelled.length) list.push({ title: 'Retail cancelled orders', value: String(cancelled.length), note: 'Check wastage or service gaps', tone: 'neutral' });
    notifications.filter(n => n.status !== 'Resolved').slice(0, 6).forEach(n => list.push({ title: n.title, value: n.status, note: `${executiveBranchDisplay(n.branch)} · ${n.details}`, tone: n.type === 'Stock Dispute' ? 'danger' : 'neutral', branch: n.branch }));
    return list;
  ...
  const dismissAlert = (a: ExecutiveAlert) => setDismissed(prev => {
    const next = new Set([...prev, `${a.title}-${a.branch ?? ''}`]);
    localStorage.setItem('executive-dismissed-alerts', JSON.stringify([...next]));
    return next;
  });
  ...
  return (
    <div className="executive-tab-stack">
      <ExecutiveToolbar>
        ...
        <button type="button" onClick={() => executiveCsvDownload('executive-alerts.csv', visible.map(a => ({ Alert: a.title, Value: a.value, Branch: a.branch || 'Business', Tone: a.tone, Details: a.note })))}><Download className="size-4" />Export</button>
      </ExecutiveToolbar>
      {/* CHANGE 9c: improved KPI metrics */}
      <section className="executive-metric-grid">
        <ExecutiveMetricCard icon={<XCircle className="size-5" />} label="Critical" value={alerts.filter(a => a.tone === 'danger').length} tone="red" />
        <ExecutiveMetricCard icon={<AlertTriangle className="size-5" />} label="Warnings" value={alerts.filter(a => a.tone === 'warning').length} tone="amber" />
        <ExecutiveMetricCard icon={<Bell className="size-5" />} label="Operational" value={alerts.filter(a => a.tone === 'neutral').length} tone="blue" />
        <ExecutiveMetricCard icon={<Store className="size-5" />} label="Branches Affected" value={branchesWithAlerts} tone="purple" />
      </section>
              ...
              <article key={`${alert.title}-${index}`} className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-start justify-between gap-3">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    {alert.branch && <em className="text-[10px] px-1.5 py-0.5 rounded-full bg-red-100 text-red-800 font-semibold mt-1 inline-block">{executiveBranchDisplay(alert.branch)}</em>}
                  </div>
                </div>
              ...
              <article key={`${alert.title}-${index}`} className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start justify-between gap-3">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    {alert.branch && <em className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-800 font-semibold mt-1 inline-block">{executiveBranchDisplay(alert.branch)}</em>}
                  </div>
                </div>
              ...
              <article key={`${alert.title}-${index}`} className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-start justify-between gap-3">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    {alert.branch && <em className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-200 text-slate-700 font-semibold mt-1 inline-block">{executiveBranchDisplay(alert.branch)}</em>}
                  </div>
                </div>
      ...
      {visible.length === 0 && <EmptyExecutiveState title="No matching executive alerts" message="Critical issues will appear here automatically from sales, credit, stock, closure and dispatch flows." />}
    </div>
  );
}

// ── Purchases & Store Visibility Tab ─────────────────────────────────────────
function ExecutivePurchasesTab() {
  const { invoices, load } = useInvoiceStore();
  const { orders: purchaseOrders, load: loadOrders } = usePurchaseOrderStore();
  const [fromDate, setFromDate] = useState(executiveDateInput(executivePresetStart('30d')));
  const [toDate, setToDate] = useState(executiveDateInput());
  const [query, setQuery] = useState('');
  const [status, setStatus] = useState('all');
  const [dbLines, setDbLines] = useState<ExecutiveStorePurchaseLine[]>([]);
  ...
  const fetchStoreInvoiceLines = useCallback(async () => {
    ...
    const rows: ExecutiveStorePurchaseLine[] = (data as Array<Record<string, unknown>>).flatMap((invoice) => {
  ...
  const fallbackLines: ExecutiveStorePurchaseLine[] = useMemo(() => invoices.flatMap((invoice, index) => invoice.lineItems.map((line, lineIndex) => ({
  ...
  const filtered = lines.filter(line => executiveInRange(line.purchaseDate, from, to) && (status === 'all' || line.purchaseStatus === status || line.stockSyncStatus === status) && `${line.supplierName} ${line.invoiceNumber} ${line.itemName}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <div className="executive-tab-stack">
      <ExecutiveToolbar>
        ...
        <div className="executive-search"><Search className="size-4" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Supplier, invoice or item" /></div>
        ...
        <button type="button" onClick={() => executiveCsvDownload('executive-store-purchases.csv', filtered.map(line => ({ Supplier: line.supplierName, Invoice: line.invoiceNumber, Date: executiveFmtDate(line.purchaseDate), Item: line.itemName, Quantity: line.quantity, Unit: line.unit, Rate: line.rate, Total: line.total, Paid: line.paidAmount, Balance: line.balanceAmount, PaymentMethod: line.paymentMethod, PurchaseStatus: line.purchaseStatus, StockSyncStatus: line.stockSyncStatus, Remarks: line.remarks })))}><Download className="size-4" />Export</button>
      </ExecutiveToolbar>
      ...
      <div className="flex flex-wrap gap-1.5">
        {[
          ...
          <button key={p.label} onClick={() => {
            ...
            setFromDate(executiveDateInput(d)); setToDate(executiveDateInput());
      ...
      <section className="executive-metric-grid">
        <ExecutiveMetricCard icon={<ShoppingBag className="size-5" />} label="Total Purchases" value={formatCurrency(totalPurchase)} sub="Store dashboard purchase invoices" tone="purple" />
        <ExecutiveMetricCard icon={<Banknote className="size-5" />} label="Paid Amount" value={formatCurrency(paid)} sub="Supplier payments captured" tone="green" />
        <ExecutiveMetricCard icon={<WalletCards className="size-5" />} label="Pending Payable" value={formatCurrency(pending)} sub="Needs payment follow-up" tone={pending ? 'amber' : 'green'} />
        <ExecutiveMetricCard icon={<Package className="size-5" />} label="Stock Sync Pending" value={filtered.filter(line => line.stockSyncStatus !== 'Synced').length} sub="Prevent missing stock" tone="red" />
        <ExecutiveMetricCard icon={<Receipt className="size-5" />} label="Purchase Orders" value={purchaseOrders.length} sub="Store purchase orders" tone="blue" />
      </section>

      <section className="executive-ops-grid compact">
        <div className="executive-panel">
          <div className="executive-panel-head"><div><span>Supplier-wise Purchase Amount</span><h3>Store purchase concentration</h3></div></div>
          <ResponsiveContainer width="100%" height={240}>
        ...
        {paymentStatusData.length > 0 && (
          <div className="executive-panel">
            <div className="executive-panel-head"><div><span>Payment Status</span><h3>Paid vs Pending</h3></div></div>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
      ...
      <section className="executive-panel">
        <div className="executive-panel-head"><div><span>Item-wise Purchase Quantity</span><h3>Material movement</h3></div></div>
        <ResponsiveContainer width="100%" height={240}>
      ...
      <section className="executive-table-card">
        <div className="executive-panel-head"><div><span>Store Purchase Register</span><h3>Supplier, invoice, item, payment and stock sync visibility</h3></div></div>
        <div className="overflow-x-auto">
          <table className="executive-data-table">
            ...
            <tbody>
              {filtered.map(line => (
                <tr key={line.id}>
                  <td><strong>{line.supplierName}</strong><span>{line.invoiceNumber}</span></td><td>{executiveFmtDate(line.purchaseDate)}</td><td>{line.itemName}</td><td>{line.quantity} {line.unit}</td><td>{formatCurrency(line.rate)}</td><td>{formatCurrency(line.total)}</td><td>{formatCurrency(line.paidAmount)}</td><td>{formatCurrency(line.balanceAmount)}</td><td>{line.paymentMethod}</td><td><em className="executive-status warn">{line.purchaseStatus}</em></td><td><em className={cn('executive-status', line.stockSyncStatus === 'Synced' ? 'ok' : 'warn')}>{line.stockSyncStatus}</em></td><td>{line.remarks}</td>
                </tr>
              ))}
              {!filtered.length && <tr><td colSpan={12}><EmptyExecutiveState title="No store purchases found" message="Create store invoices or adjust filters to see purchase visibility." /></td></tr>}
            </tbody>
          </table>
        </div>
      </section>
...
function ExecutiveStockVarianceTab() {
    ...
    <section className="executive-section">
      <div className="mb-4">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-muted-foreground">
          Difference between system stock and physical stock count after Outlet Management confirmation
        </p>
      ...
      <ExecutiveToolbar>
        <button
          type="button"
          onClick={() =>
            executiveCsvDownload(
              'executive-stock-variance.csv',
              rows.map(row => ({
                Date: executiveFmtDateTime(row.createdAt),
                Branch: row.branch,
                Report: row.reportNo,
                Item: row.itemName,
                Unit: row.unit || '',
      ...
      <section className="executive-metric-grid">
        <ExecutiveMetricCard icon={<AlertTriangle className="size-5" />} label="Variance Lines" value={rows.length} tone="amber" />
        <ExecutiveMetricCard icon={<ArrowDownRight className="size-5" />} label="Short Count" value={shortCount} tone="red" />
        <ExecutiveMetricCard icon={<ArrowUpRight className="size-5" />} label="Excess Count" value={excessCount} tone="blue" />
      </section>
      <section className="executive-table-card">
        <table>
          <thead>
          ...
          <tbody>
            {rows.map(row => (
              <tr key={row.id}>
                <td>{executiveFmtDateTime(row.createdAt)}</td>
                <td>{executiveBranchDisplay(row.branch)}</td>
                <td><strong>{row.reportNo}</strong></td>
                <td>{row.itemName}</td>
                <td>{row.systemQty} {row.unit}</td>
                <td>{row.physicalQty} {row.unit}</td>
                <td><em className={cn('executive-status', row.difference === 0 ? 'ok' : row.difference > 0 ? 'danger' : 'warn')}>{row.difference}</em></td>
              ...
              <tr>
                <td colSpan={9}>
                  <EmptyExecutiveState
                    ...
                    message="Variance lines will appear here once Outlet Management confirms a receiver stock-count report."
                  />
                </td>
              </tr>
            )}
...
// ── Main Export ───────────────────────────────────────────────────────────────
type ExecutiveControlModuleTab =
  | 'branches'
  | 'sales'
  | 'credit'
  | 'purchases'
...
export default function ExecutiveControlModule() {
  const [searchParams, setSearchParams] = useSearchParams();
  const requestedTab = searchParams.get('tab') as ExecutiveControlModuleTab | null;
  const executiveTabIds = useMemo<ExecutiveControlModuleTab[]>(() => ['branches', 'sales', 'credit', 'purchases', 'closure', 'variance', 'alerts', 'attendance', 'waste', 'complaints', 'audit'], []);
  const initialTab = requestedTab && executiveTabIds.includes(requestedTab) ? requestedTab : 'branches';
  const [tab, setTab] = useState<ExecutiveControlModuleTab>(initialTab);
  const selectTab = (next: ExecutiveControlModuleTab) => {
    setTab(next);
    setSearchParams(next === 'branches' ? {} : { tab: next });
  };

  useEffect(() => {
    if (requestedTab && executiveTabIds.includes(requestedTab) && requestedTab !== tab) {
      setTab(requestedTab);
    }
  }, [requestedTab, executiveTabIds, tab]);

  const tabs: Array<{ id: ExecutiveControlModuleTab; label: string; icon: React.ReactNode; hint: string }> = [
    { id: 'branches',   label: 'Branch Overview',    icon: <Store         className="size-4" />, hint: 'Retail, primary outlet, secondary outlet, wholesale' },
    ...
    { id: 'alerts',     label: 'Executive Alerts',       icon: <Bell          className="size-4" />, hint: 'Actionable risks' },
  ...
  return (
    <main className="executive-dashboard-body px-4 py-5 sm:px-6 xl:px-8">
      {tab === 'branches'   && <BranchOverviewTab />}
      {tab === 'sales'      && <SalesOverviewTab />}
      {tab === 'credit'     && <ExecutiveCreditTab />}
      {tab === 'purchases'  && <ExecutivePurchasesTab />}
      {tab === 'closure'    && <ExecutiveDailyClosureTab />}
      {tab === 'variance'   && <ExecutiveStockVarianceTab />}
      {tab === 'alerts'     && <ExecutiveAlertsTab />}
      {tab === 'attendance' && <AttendanceSalaryTab />}
      {tab === 'waste'      && <WasteLogsTab />}
      {tab === 'complaints' && <ExecutiveComplaintsTab />}
      {tab === 'audit'      && <ExecutiveAuditTab />}
    </main>
  );
}
