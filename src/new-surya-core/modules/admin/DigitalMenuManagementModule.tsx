...
import { RETAIL_CONFIG, TABLE_NUMBERS } from '@/constants/config';
...
export default function QRMenuPage() {
  ...
  useEffect(() => {
    if (selectedTable === 'general') {
      setMenuUrl(`${window.location.origin}/retail-order`);
    } else {
      setMenuUrl(`${window.location.origin}/retail-order?table=${selectedTable}`);
    }
  }, [selectedTable]);
  ...
  const downloadQR = () => {
    ...
    ctx.fillText(RETAIL_CONFIG.name, out.width / 2, size + pad + 30);
    ...
    link.download = selectedTable === 'general'
      ? 'new-surya-order-qr.png'
      : `new-surya-table-${selectedTable}-qr.png`;
  ...
  const downloadAllTables = async () => {
    for (let idx = 0; idx < TABLE_NUMBERS.length; idx++) {
      ...
      const url = `${window.location.origin}/retail-order?table=${tableNum}`;
      ...
      const link = document.createElement('a');
      link.download = `new-surya-table-${tableNum}-qr.png`;
      link.href = out.toDataURL('image/png');
      link.click();
      ...
      <div className="px-4 space-y-4">
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          <div className="flex flex-col items-center py-8 px-6 gap-4">
            <p className="font-display text-lg font-bold text-foreground">{RETAIL_CONFIG.name}</p>
            ...
            <p className="text-xs font-body text-muted-foreground text-center">{RETAIL_CONFIG.type} • {RETAIL_CONFIG.hours}</p>
            <p className="text-[10px] font-body text-muted-foreground text-center">{RETAIL_CONFIG.address}</p>
          </div>
