...
export default function DataHealthBanner() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('retail:data-error', failed);
    window.addEventListener('retail:data-recovered', recovered);
    window.addEventListener('retail:session-expired', expired);
    return () => {
      window.removeEventListener('retail:data-error', failed);
      window.removeEventListener('retail:data-recovered', recovered);
      window.removeEventListener('retail:session-expired', expired);
    };
  }, [logout]);
