...
export default function AdminPlanningTab() {
  const catalog = useBranchCatalogStore(state => state.items.PRIMARY_OUTLET);
  const loading = useBranchCatalogStore(state => state.loading.PRIMARY_OUTLET);
  ...
  useEffect(() => { void loadCatalog('PRIMARY_OUTLET'); }, [loadCatalog]);
  ...
  const addCatalogItem = (item: BranchCatalogItem) => {
    const key = `primary_outlet-${item.barcode}`;
  ...
  const sendPlan = async () => {
    ...
    try {
      ...
      await submitOrder(items, currentUser?.displayName || currentUser?.username || 'Admin', 'PRIMARY_OUTLET', planNotes);
      setLines([]);
      setNotes('');
  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
        <div className="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center">
          <div className="relative min-w-0 flex-1">
            ...
            <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search outlet item or barcode" className="h-11 w-full rounded-xl border border-slate-200 pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-emerald-200" />
          </div>
          ...
          <button type="button" title="Refresh Outlet items" onClick={() => void loadCatalog('PRIMARY_OUTLET', true)} disabled={loading} className="grid size-11 place-items-center rounded-xl border border-slate-200 bg-white hover:bg-slate-50 disabled:opacity-50">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white">
        ...
        {lines.length === 0 ? <p className="p-8 text-center text-sm font-semibold text-slate-500">Add outlet or custom items to create a production plan.</p> : <div className="divide-y divide-slate-100">{lines.map(line => {
          const step = line.dispatchUnit === 'kg' ? 0.25 : 1;
          return <div key={line.key} className="grid gap-3 px-4 py-3 sm:grid-cols-[1fr_auto_auto] sm:items-center"><div><p className="font-bold text-slate-950">{line.itemName}</p><p className="text-xs text-slate-500">{line.category} | {line.isCustom ? 'Custom item' : 'outlet catalogue'}</p></div><div className="flex items-center gap-1"><button type="button" title="Decrease quantity" onClick={() => setQuantity(line.key, line.quantity - step)} className="grid size-9 place-items-center rounded-lg border"><Minus className="size-4" /></button><input value={line.quantity} onChange={event => setQuantity(line.key, Number(event.target.value))} type="number" min={step} step={step} className="h-9 w-24 rounded-lg border px-2 text-center text-sm font-black" /><span className="w-7 text-xs font-bold text-slate-500">{line.dispatchUnit}</span><button type="button" title="Increase quantity" onClick={() => setQuantity(line.key, line.quantity + step)} className="grid size-9 place-items-center rounded-lg border"><Plus className="size-4" /></button></div><button type="button" title="Remove item" onClick={() => setLines(current => current.filter(item => item.key !== line.key))} className="grid size-9 place-items-center rounded-lg text-red-600 hover:bg-red-50"><Trash2 className="size-4" /></button></div>;
        })}</div>}
