// src/components/admin/ExecutiveCreditTab.tsx
...
const BRANCH_COLORS: Record<Branch, string> = {
  Retail:  '#4B9F6E',
  SECONDARY_OUTLET: '#5BA3C9',
  PRIMARY_OUTLET:   '#C5973E',
  Wholesale: '#2D7D6F',
};
...
function BranchCreditCard({ branch, sales }: { branch: Branch; sales: CreditSale[] }) {
  ...
  const BRANCH_PILL: Record<Branch, string> = {
    Retail:  'bg-green-50 border-green-200 text-green-700',
    SECONDARY_OUTLET: 'bg-blue-50 border-blue-200 text-blue-700',
    PRIMARY_OUTLET:   'bg-amber-50 border-amber-200 text-amber-700',
    Wholesale: 'bg-emerald-50 border-emerald-200 text-emerald-700',
  };
...
// ── Main Component ─────────────────────────────────────────────────────────────
export default function ExecutiveCreditTab() {
  ...
  // CHANGE 6: CSV export
  const exportCredits = () => executiveCsvDownload('executive-credits.csv', filtered.map(c => ({
    Branch:      c.branch,
    ...
    BillNo:      c.billNo || '-',
    SoldBy:      c.soldBy,
    Date:        executiveFmtDate(c.createdAt),
  })));
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        <span className="text-[10px] text-muted-foreground">{executiveFmtDate(sale.createdAt)}</span>
                        ...
                        {sale.dueDate && (
                          <span className={cn('text-[10px]', isOverdue ? 'text-red-600 font-semibold' : 'text-muted-foreground')}>
                            Due: {executiveFmtDate(sale.dueDate)}
                          </span>
                        )}
                      </div>
