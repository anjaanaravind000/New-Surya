...
import { AdvancePaymentPanel } from '@/modules/branch/RetailBillingModule';
...
export default function OrderCard({ order, showActions = false, counterOpenedToday = true }: OrderCardProps) {
  ...
  const handleSinglePayment = async (pt: PaymentType) => {
    ...
    if (!counterOpenedToday) {
      setPaymentError('Counter is not opened. Open Retail Daily Closure, then Counter Open before collecting payment.');
      return;
    }
    try {
  ...
  const handleSplitPayment = async () => {
    if (!counterOpenedToday) {
      setSplitError('Counter is not opened. Open Retail Daily Closure, then Counter Open before collecting payment.');
      return;
    }
          ...
          <div className="px-3.5 py-2.5 border-t border-border flex gap-2">
            {order.status === 'pending' && (
              <button
                ...
                className="flex-1 py-2.5 rounded-lg surya-gradient text-primary-foreground text-sm font-body font-bold active:scale-[0.97] transition-transform"
              >
              ...
              <>
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-sm font-body font-semibold border transition-all active:scale-95 ${
                        splitMethods.includes(m)
                          ? 'surya-gradient text-primary-foreground border-transparent'
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-lg surya-gradient text-primary-foreground text-sm font-body font-bold active:scale-95 disabled:opacity-40 disabled:pointer-events-none"
                  >
                    Confirm Split
                  </button>
        ...
        {showDiscount && (
          <div className="px-3.5 py-3 border-t border-border bg-accent/5 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleApplyDiscount} className="px-4 py-2 rounded-md surya-gradient text-primary-foreground text-sm font-body font-bold active:scale-95">Apply</button>
            </div>
