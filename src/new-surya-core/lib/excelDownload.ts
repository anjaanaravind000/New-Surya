import { brandExportValue, replaceVisibleBranding, sanitizeExportFilename } from './visibleBranding';
...
function worksheetXml(sheet: ExcelWorksheet) {
  ...
  const cellXml = (header: string, value: ExcelExportValue) => {
    ...
    return `<Cell><Data ss:Type="String">${escapeXml(String(brandExportValue(value) ?? ""))}</Data></Cell>`;
  };
  ...
  <Table ss:ExpandedColumnCount="${headers.length}" ss:ExpandedRowCount="${safeRows.length + 1}" x:FullColumns="1" x:FullRows="1">
   ...
   <Row ss:StyleID="Header" ss:AutoFitHeight="1">${headers.map((header) => `<Cell><Data ss:Type="String">${escapeXml(replaceVisibleBranding(header))}</Data></Cell>`).join("")}</Row>
