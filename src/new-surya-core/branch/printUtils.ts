...
import { replaceVisibleBranding } from '@/lib/visibleBranding';

export const BRANCH_PRINT_COMPLETE_EVENT = 'new-surya:branch-print-complete';
...
export function printHtml(title: string, body: string) {
  const brandedTitle = replaceVisibleBranding(title);
  const brandedBody = replaceVisibleBranding(body);
  const w = window.open('', '_blank', 'width=920,height=900');
  if (w) {
    w.document.write(
      `<!doctype html><html><head><title>${brandedTitle}</title><style>
        ...
        *{box-sizing:border-box}
        ...
        main:before{content:"${brandedTitle}";display:block;margin:-4px 0 18px;padding:14px 16px;border-radius:24px;background:#0f172a;color:#fff;font-size:20px;font-weight:900;letter-spacing:.02em}
      ...
      </style></head><body><main>${brandedBody}</main><script>window.onload=()=>window.print()</script></body></html>`,
    );
    w.document.close();
  }
}

// ─── SECONDARY_OUTLET receipt-style counter bill ─────────────────────────────────────────
// Matches the physical receipt format from the PRIMARY_OUTLET/SECONDARY_OUTLET receipt image.
function printSecondaryOutletReceiptBill(bill: BranchBillRecord, duplicate = false, target?: Window | null) {
  ...
  const html = `<!doctype html><html><head><title>${duplicate ? 'DUPLICATE BILL' : 'BILL'} ${bill.billNo}</title><style>
    ...
    <div class="brand">PRIMARY_OUTLET</div>
    <div class="c sub">SWEETS &amp; BAKES</div>
    <div class="c sub">SECONDARY_OUTLET FOODS LLP</div>
    ...
    <div class="dash"></div>
    <div class="c bold" style="font-size:13px">SECONDARY_OUTLET FOODS LLP</div>
    <div class="c sub">#109/1C, Wholesale main Road, Berigai,</div>
...
// ─── Full-format counter bill (PRIMARY_OUTLET style / tax invoice) ───────────────────────
function printPrimaryOutletCounterBill(bill: BranchBillRecord, duplicate = false, target?: Window | null) {
  const returnBill = bill as BranchBillRecord & { _isReturn?: boolean; _originalBillNo?: string; _returnReason?: string };
  const title = returnBill._isReturn ? 'RETURN BILL' : duplicate ? 'DUPLICATE BILL' : 'ORIGINAL BILL';
  const business = {
    name: 'New Surya Bakery',
    lines: ['404, Bagalur Main Road, Berigai Bus Stand, Berigai, Shoolagiri Taluk', 'Krishnagiri, Tamil Nadu, Wholesale-635105', 'Phone: 9942266779, 9095445444'],
...
export function printAccountingVoucher(input: AccountingVoucherPrintInput) {
  ...
  const html = `<!doctype html><html><head><title>${safeHtml(input.voucherType)} ${safeHtml(input.voucherNo)}</title><style>
    ...
    <header class="company"><h1>SRI NANJUNDESHWARA BAKERY</h1><p>Berigai, Krishnagiri, Tamil Nadu</p><p>Retail Outlet Accounts</p></header>
...
export async function printCounterBill(bill: BranchBillRecord, duplicate = false) {
  ...
  if (bill.branch === 'SECONDARY_OUTLET') printSecondaryOutletReceiptBill(bill, isDuplicate, target);
  else printPrimaryOutletCounterBill(bill, isDuplicate, target);
}
