// src/branch?suite=branch-operationsItems.ts
// Auto-generated from PRIMARY_OUTLET_FINAL_PRICE_LIST.xlsx
// Used by PRIMARY_OUTLET and wholesale outletes for billing and admin item management.

export interface PrimaryOutletItem {
  barcode: number;
  name: string;
  price: number;
  uom: 'Nos' | 'Kgs';
  category: PrimaryOutletCategory;
}

export type PrimaryOutletCategory =
  | 'Bread & Buns'
  | 'Cakes (by kg)'
  | 'Biscuits & Cookies'
  | 'Chocolates'
...
export const PRIMARY_OUTLET_CATEGORIES: PrimaryOutletCategory[] = [
  'Bread & Buns',
  'Cakes (by kg)',
  'Biscuits & Cookies',
  'Chocolates',
...
export const PRIMARY_OUTLET_ITEMS: PrimaryOutletItem[] = [
  // ── Bread & Buns ──────────────────────────────────────────────────────────
  ...
  // ── Chocolates ────────────────────────────────────────────────────────────
  { barcode: 1036, name: 'NEW SURYA CHOCOLATES', price: 1500, uom: 'Kgs', category: 'Chocolates' },
  { barcode: 1037, name: 'NEW SURYA LOLLIPOP',   price: 15,   uom: 'Nos', category: 'Chocolates' },
...
// Branches that use the PRIMARY_OUTLET price list
export const PRIMARY_OUTLET_PRICE_LIST_BRANCHES = ['PRIMARY_OUTLET', 'Wholesale'] as const;

// Lookup by barcode
export function getPrimaryOutletItemByBarcode(barcode: number): PrimaryOutletItem | undefined {
  return PRIMARY_OUTLET_ITEMS.find((i) => i.barcode === barcode);
}

// Lookup by name (case-insensitive)
export function getPrimaryOutletItemByName(name: string): PrimaryOutletItem | undefined {
  const lower = name.toLowerCase().trim();
  return PRIMARY_OUTLET_ITEMS.find((i) => i.name.toLowerCase() === lower);
}
