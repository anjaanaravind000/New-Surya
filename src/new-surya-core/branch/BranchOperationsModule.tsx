// src/branch/BranchOperationsModule.tsx
...
import { useOutletTabStripStore } from '@/stores/outletTabStripStore';
...
export default function BranchOperationsModule({ branch }: Props) {
  ...
  const tabStripExpanded = useOutletTabStripStore((s) => s.expanded);
  ...
  const isAdminUser = role === 'admin' || role === 'executive' || (branch === 'PRIMARY_OUTLET' && role === 'branch_incharge_primary') || (branch === 'SECONDARY_OUTLET' && role === 'branch_incharge_secondary');
  const isPrimaryOutletAdmin = role === 'branch_incharge_primary';
  const isSecondaryOutletAdmin = role === 'branch_incharge_secondary';
  const canViewReports = branch === 'SECONDARY_OUTLET'
    ? isSecondaryOutletAdmin
    : branch === 'PRIMARY_OUTLET'
      ? isPrimaryOutletAdmin
      : isAdminUser;
  const canViewSalespersonReport = branch === 'PRIMARY_OUTLET' ? isPrimaryOutletAdmin : branch === 'SECONDARY_OUTLET' ? isSecondaryOutletAdmin : isAdminUser;
  ...
  useEffect(() => {
    ...
    window.addEventListener('retail:refresh-branch-dashboard', handleHeaderRefresh);
    return () => window.removeEventListener('retail:refresh-branch-dashboard', handleHeaderRefresh);
  }, [refreshAll]);
              ...
              .map((t) => {
                ...
                return (
                  <button
                    key={t.id}
                    type="button"
                    onClick={() => { openTab(t.id); useOutletTabStripStore.getState().collapse(); }}
                    className={cn(
