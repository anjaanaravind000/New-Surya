...
const BRANCH_OPS_STATE_KEY = "new-surya-branch-ops-v1";
...
function getSessionBranch(): Branch | null {
  try {
    ...
    const raw = sessionStorage.getItem('new-surya-auth')
      ?? localStorage.getItem('new-surya-auth');
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { state?: { currentUser?: { role?: string } } };
    const role = parsed?.state?.currentUser?.role ?? '';
    if (role === 'branch_primary' || role === 'branch_incharge_primary' || role === 'stock_audit_primary') return 'PRIMARY_OUTLET';
    if (role === 'branch_secondary' || role === 'branch_incharge_secondary' || role === 'stock_audit_secondary') return 'SECONDARY_OUTLET';
    if (role === 'branch_wholesale') return 'Wholesale';
    return null; // admin/admin?suite=executive-control roles load all branches — handled below
  } catch {
    return null;
  }
}
...
const branchOpsSupabaseStorage: PersistStorage<BranchOpsState> = {
  getItem: async (name) => {
    ...
    // Admin/admin?suite=executive-control roles (sessionBranch === null) still load everything.
        ...
        const varianceNotification: BranchNotification | null =
          varianceRows.length > 0
            ? {
                ...
                details: `${report.reportNo} confirmed by ${confirmedBy}. ${varianceRows.length} item differences sent to Executive/Administration.`,
                createdAt: now,
