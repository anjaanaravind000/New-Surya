...
const PRIMARY_OUTLET_BRANCHES = ['PRIMARY_OUTLET', 'Wholesale'] as const;
...
function ManualStockUpdate({ branch, branchStock }: { branch: Branch; branchStock: StockItem[] }) {
  ...
  const isPRIMARY_OUTLET = (PRIMARY_OUTLET_BRANCHES as readonly string[]).includes(branch);
  const { items: catalogue, categories: catalogCategories } = useOperationalBranchCatalog(isPRIMARY_OUTLET ? 'PRIMARY_OUTLET' : 'SECONDARY_OUTLET');
...
export function StockTab({ branch, branchStock, branchIncoming, branchThresholds, loading, stockMismatches, allowManualUpdate = true }: Props) {
  ...
  const PRIMARY_OUTLET_BRANCHES_CONST = ['PRIMARY_OUTLET', 'Wholesale'] as const;
  const isPRIMARY_OUTLETBranch = (PRIMARY_OUTLET_BRANCHES_CONST as readonly string[]).includes(branch);
  const { items: liveCatalogue } = useOperationalBranchCatalog(isPRIMARY_OUTLETBranch ? 'PRIMARY_OUTLET' : 'SECONDARY_OUTLET');
