...
function isPrimaryOutletFlexibleStockItem(branch: Branch, item?: Pick<BillingItem, 'category'> | null) {
  if (branch !== 'PRIMARY_OUTLET' || !item) return false;
...
export default function BranchBillingProTab({
  ...
  const isAdmin = ['admin', 'branch_incharge_primary', 'branch_incharge_secondary', 'executive'].includes(currentUser?.role || '');
  const isSECONDARY_OUTLET = branch === 'SECONDARY_OUTLET';
  const catalogBranch = isSECONDARY_OUTLET ? 'SECONDARY_OUTLET' : 'PRIMARY_OUTLET';
  ...
  const setItemQuantity = useCallback((item: BillingItem, amount = 1) => {
    ...
    const allowInsufficientStock = isPrimaryOutletFlexibleStockItem(branch, item);
  ...
  const submitQtyPopup = useCallback(() => {
    ...
    if (!isPrimaryOutletFlexibleStockItem(branch, qtyPopupItem) && amount > available) {
  ...
  const commitCartQuantity = (itemName: string) => {
    ...
    if (!isPrimaryOutletFlexibleStockItem(branch, catalogItem) && nextQty > available) {
  ...
  const validateCheckout = () => {
    ...
    for (const item of cart) {
      const catalogItem = items.find((catalog) => catalog.barcode === item.barcode || catalog.name === item.itemName);
      const available = Number(stockAvailable(branchStock, stockMap, item.itemName, item.barcode));
      if (!isPrimaryOutletFlexibleStockItem(branch, catalogItem)) {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      const flexibleStockShortfall = branch === 'PRIMARY_OUTLET' && canonicalItems.some((line) => {
        const catalogItem = items.find((item) => item.barcode === line.barcode || item.name === line.itemName);
        if (!isPrimaryOutletFlexibleStockItem(branch, catalogItem)) return false;
        return line.quantity > Number(stockAvailable(branchStock, stockMap, line.itemName, line.barcode));
      });

      let checkoutRpc: 'v4' | 'v3' | 'v2' | 'base' = branch === 'PRIMARY_OUTLET' ? 'v4' : 'v3';
      const checkoutResponse = branch === 'PRIMARY_OUTLET'
      ...
      if (rpcError) {
        if (flexibleStockShortfall && isRpcUnavailableError(rpcError, 'complete_branch_checkout_canonical_v4')) {
          throw new Error('Mix & Combo checkout is temporarily unavailable while the Supabase API refreshes. Retry the bill once; stock and discount values have not been changed.');
        }
  ...
  return (
    <div className="branch-new-surya-pos h-full min-h-0 overflow-hidden rounded-[1.35rem] border border-slate-200 bg-slate-100 shadow-xl shadow-slate-200/70">
            ...
            <div className="mt-1 flex items-center gap-2 rounded-xl border-2 border-slate-200 bg-slate-50 px-2.5 py-0.5 focus-within:border-amber-400">
              ...
              <div className="relative flex-1">
                <input
                  ...
                  placeholder={billingInputMode === 'barcode' ? `Scan ${branch === 'PRIMARY_OUTLET' ? '1001...' : '2001...'} barcode and press Enter` : 'F12 - search item name or barcode'}
      ...
      {showQtyPopup && qtyPopupItem && (
        <QuantityPopup
          ...
          stockMap={stockMap}
          allowInsufficientStock={isPrimaryOutletFlexibleStockItem(branch, qtyPopupItem)}
