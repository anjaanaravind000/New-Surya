...
export function PaymentModeEditTab({ branch }: { branch: Branch }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => !value
      ...
      || (!isSECONDARY_OUTLET && row.salesperson.toLowerCase().includes(value))
      || row.biller.toLowerCase().includes(value));
  }, [isSECONDARY_OUTLET, query, rows]);
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white shadow-sm">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-3 py-2">
        ...
        <div className="relative min-w-[220px] flex-1 sm:max-w-sm"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isSECONDARY_OUTLET ? 'Search bill or cashier' : 'Search bill, salesperson or cashier'} className="h-9 w-full rounded-xl border-2 border-slate-200 bg-slate-50 pl-9 pr-3 text-sm font-bold outline-none focus:border-blue-400" /></div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2">
        <table className={cn('w-full text-sm', isSECONDARY_OUTLET ? 'min-w-[920px]' : 'min-w-[1040px]')}>
          <thead className="sticky top-0 z-10 bg-slate-50 text-left text-[10px] font-black uppercase tracking-wide text-slate-500"><tr><th className="p-2">Bill</th><th className="p-2">Date</th>{!isSECONDARY_OUTLET && <th className="p-2">Salesperson</th>}<th className="p-2">Cashier</th><th className="p-2 text-right">Amount</th><th className="p-2">Current</th><th className="p-2">Correct Allocation</th><th className="p-2 text-right">Action</th></tr></thead>
          <tbody>{loading ? <tr><td colSpan={isSECONDARY_OUTLET ? 7 : 8} className="p-8 text-center font-bold text-slate-500">Loading bill history...</td></tr> : filtered.length === 0 ? <tr><td colSpan={isSECONDARY_OUTLET ? 7 : 8} className="p-8 text-center font-bold text-slate-500">No bills found.</td></tr> : filtered.map((row) => {
            const valid = draftTotal(row.id) === roundMoney(row.total);
            return <tr key={row.id} className="border-t border-slate-100 align-middle">
              <td className="p-2 font-black text-slate-950">{row.billNo}</td><td className="p-2 text-xs text-slate-600">{new Date(row.createdAt).toLocaleString('en-IN')}</td>{!isSECONDARY_OUTLET && <td className="p-2 font-bold">{row.salesperson}</td>}<td className="p-2">{row.biller}</td><td className="p-2 text-right font-black">{money(row.total)}</td><td className="p-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black uppercase text-slate-700">{row.mode}</span></td>
