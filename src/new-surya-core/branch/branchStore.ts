...
export const useBranchStore = create<BranchState>((set, get) => ({
  stock:           { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  sales:           { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  incoming:        { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  advanceOrders:   { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  creditSales:     { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  creditPayments:  { Retail: [], SECONDARY_OUTLET: [], PRIMARY_OUTLET: [], Wholesale: [] },
  thresholds:      { Retail: {}, SECONDARY_OUTLET: {}, PRIMARY_OUTLET: {}, Wholesale: {} },
  stockMismatches: [],
  loading:         false,
  lastCleanedAt:   null,
  lastSyncedAt:    { Retail: null, SECONDARY_OUTLET: null, PRIMARY_OUTLET: null, Wholesale: null } as Record<Branch, number | null>,
  ...
  fetchBranchData: async (branch, force = false) => {
    ...
    try {
      ...
      const catalogBranch = branch === 'SECONDARY_OUTLET' ? 'SECONDARY_OUTLET' : branch === 'PRIMARY_OUTLET' || branch === 'Wholesale' ? 'PRIMARY_OUTLET' : null;
      ...
      const [
        ...
        // PRIMARY_OUTLET/SECONDARY_OUTLET/Wholesale prices already come from the cached live branch
        // catalogue. Only Retail still needs the legacy bakery price fallback.
        catalogBranch
  ...
  fetchAllBranches: async () => {
    await Promise.all((['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'] as Branch[]).map((b) => get().fetchBranchData(b)));
  },
  ...
  //   duplicate counting on Executive Dashboard.
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  // ── PRIMARY_OUTLET / Wholesale sale — items come from price list, not stock requirement ──
  // Deducts stock when available, logs a mismatch when stock is 0 / insufficient.
  recordPrimaryOutletSale: async (branch, itemName, qty, soldBy, paymentMethod, unitPrice, billNo, itemBarcode) => {
    const now = new Date().toISOString();
    const requestedStockName = normalizeStockName(itemName);
    const currentStock = get().stock[branch].find((stockItem) =>
      itemBarcode != null && stockItem.itemBarcode != null
    ...
    const { data: newQtyRpc, error: rpcErr } = await decrementBranchStockStrict(branch, stockItemName, qty, resolvedBarcode);
    if (rpcErr) {
      console.error('[recordPrimaryOutletSale] stock RPC error:', rpcErr.message);
    ...
    // Sync from DB after PRIMARY_OUTLET sale to pick up any concurrent changes
  ...
  manualUpdateStock: async (branch, itemName, quantity, updatedBy, itemBarcode, audit) => {
    ...
    if (existing) {
      ...
      // executives can see who changed what, when, and by how much. Requires a
  ...
  recordCreditSale: async (branch, sale, options = {}) => {
    ...
    // FIX (MD Bug #8): skip branch_sales write for branch='Retail'. Retail credit orders are
    // already fully recorded in the orders table (source of truth for Retail sales). Writing
    // to branch_sales creates dead data today and would cause double-counting if a future
    // Retail item-wise report is ever built on branch_sales (mirroring the SECONDARY_OUTLET/PRIMARY_OUTLET pattern).
    const shouldWriteSalesRows = options.writeSalesRows !== false && branch !== 'Retail';
