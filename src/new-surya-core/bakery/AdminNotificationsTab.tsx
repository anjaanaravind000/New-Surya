...
> = {
  ...
  primary_outlet_purchase_invoice_revision: {
    label: 'Purchase Invoice Revision',
    icon: RefreshCw,
...
function NotificationDetailModal({
  ...
  const renderDetail = () => {
    ...
    if (notification.type === 'primary_outlet_purchase_invoice_revision' && m) {
...
export default function AdminNotificationsTab() {
  ...
  const revisionCount    = notifications.filter(n => n.type === 'primary_outlet_purchase_invoice_revision').length;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-8 gap-2">
        {[
          ...
          { label: 'Purchase Revisions', value: revisionCount, color: revisionCount > 0 ? 'text-orange-600' : 'text-muted-foreground', bg: revisionCount > 0 ? 'bg-orange-50 border-orange-200' : '' },
      ...
      <div className="flex items-center gap-2">
        <div className="flex gap-1.5 overflow-x-auto flex-1 pb-0.5">
          {([
            ...
            { id: 'primary_outlet_purchase_invoice_revision', label: '🔄 Purchase Revisions' },
