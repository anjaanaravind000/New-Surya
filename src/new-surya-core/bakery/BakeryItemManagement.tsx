...
//
// ── Retail toggle   → full retail menu management (MenuManagement UI)
...
import MenuManagement   from '@/modules/admin/MenuManagementModule';
    ...
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      <div className="w-full bg-background rounded-t-2xl border-t border-border p-5 pb-28 space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={handleSave} disabled={saving || !dirty || !name.trim()} className="flex-1 h-10 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-bold disabled:opacity-50 flex items-center justify-center gap-1.5">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      <div className="w-full bg-background rounded-t-2xl border-t border-border p-5 pb-28 space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={onClose} className="flex-1 h-10 rounded-xl border border-border text-sm font-body font-semibold text-muted-foreground">Cancel</button>
          <button onClick={handleAdd} disabled={saving || !name.trim()} className="flex-1 h-10 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-bold disabled:opacity-50 flex items-center justify-center gap-1.5">
...
export function BakeryItemsPanel({ autoOpen = false }: { autoOpen?: boolean }) {
    ...
    <div className="pb-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="h-9 px-4 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-bold flex items-center gap-1.5 shrink-0 active:scale-95 transition"
        >
...
// ── Bakery section: Items + Primary Outlet Items + Recipe management sub-tabs ─────────────
...
function BakerySection({ autoOpenProduct = false, autoOpenRecipe = false }: { autoOpenProduct?: boolean; autoOpenRecipe?: boolean }) {
  const [subTab, setSubTab] = useState<'items' | 'primary_outlet' | 'secondary_outlet' | 'recipes'>(autoOpenRecipe ? 'recipes' : 'items');
  useEffect(() => { if (autoOpenRecipe) setSubTab('recipes'); }, [autoOpenRecipe]);

  const SUBTABS = [
    { id: 'items'   as const, label: 'Items' },
    { id: 'primary_outlet'     as const, label: 'Outlet items' },
    { id: 'secondary_outlet'   as const, label: 'Secondary Outlet items' },
  ...
  return (
    <div>
      ...
      {subTab === 'items'   && <BakeryItemsPanel autoOpen={autoOpenProduct} />}
      {subTab === 'primary_outlet'     && <PrimaryOutletItemsTab />}
      {subTab === 'secondary_outlet'   && <SecondaryOutletItemsTab />}
      {subTab === 'recipes' && <RecipeManagement autoOpen={autoOpenRecipe} />}
    </div>
  );
}
...
export default function BakeryItemManagement() {
  ...
  const [mode, setMode] = useState<'retail' | 'bakery'>(action ? 'bakery' : 'retail');
  useEffect(() => { if (action) setMode('bakery'); }, [action]);
    ...
    <div className="dashboard-screen min-h-screen bg-transparent pt-0 pb-6 px-4">
      ...
      <div className="pt-4 pb-3">
        ...
        <p className="text-xs font-body text-muted-foreground mt-0.5">
          Manage retail menu items and bakery items & recipes
        </p>
      </div>

      {/* Retail / Bakery toggle */}
      <div className="flex gap-1.5 p-1 rounded-2xl mb-5" style={{ background: 'hsl(var(--muted))' }}>
        <button
          onClick={() => setMode('retail')}
          className={cn(
            'flex-1 py-2.5 rounded-xl text-sm font-body font-semibold transition-all duration-200',
            mode === 'retail' ? 'bg-card shadow-soft text-foreground' : 'text-muted-foreground hover:text-foreground',
          )}
        >
          ☕ Retail
        </button>
      ...
      {/* Content */}
      {mode === 'retail'   && <MenuManagement embedded />}
      {mode === 'bakery' && <BakerySection autoOpenProduct={action === 'add-product'} autoOpenRecipe={action === 'add-recipe'} />}
    </div>
  );
}
