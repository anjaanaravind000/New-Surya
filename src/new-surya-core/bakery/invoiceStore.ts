...
export const useInvoiceStore = create<InvoiceState>((set, get) => ({
  ...
  load: async () => {
    ...
    try {
      ...
      // Admin/admin?suite=executive-control reads go through a role-checked RPC once the accompanying
  ...
  createInvoice: async (data) => {
    ...
    // The database trigger replaces this collision-safe placeholder with SECONDARY_OUTLET-N.
    const invoiceNumber = `SECONDARY_OUTLET-PENDING-${rand}`;
  ...
  updateInvoice: async (id, data) => {
    ...
    // Compatibility fallback for databases where the secure edit RPC has
    // not been deployed yet. Use a real numeric edit count; never pass an RPC
    // builder object into a numeric column.
    if (error && isMissingRpcError(error)) {
      ...
      const { data: updatedRows, error: directError } = await supabase
        ...
        .update({
          ...
          edited_at: editedAt,
          edit_count: (existing?.editCount ?? 0) + 1,
        })
        .eq('id', id)
        .eq('status', 'pending_review')
        .select('id');

      if (directError) return directError.message;
      if (!updatedRows?.length) return 'Only pending invoices can be edited. Refresh and try again.';
    } else if (error) {
  ...
  updateStatus: async (id, status, reviewNote) => {
    ...
    if (isMissingRpcError(result.error)) {
      result = await supabase
        ...
        .update({
          status,
          // Keep the legacy executive/purchase reporting field in sync as well.
