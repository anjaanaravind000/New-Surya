    ...
    <div className="space-y-4 pt-2">
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button onClick={handleSave} disabled={saving}
          className="flex-1 h-10 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-bold flex items-center justify-center gap-1.5 disabled:opacity-50 active:scale-95 transition-all">
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-[130px_90px_minmax(0,1fr)_auto]">
          ...
          <button onClick={handleAdd} disabled={saving || !name.trim()} className="h-10 px-4 rounded-xl surya-gradient text-primary-foreground text-sm font-body font-bold disabled:opacity-50 flex items-center justify-center gap-1.5">
...
// ── Main component ─────────────────────────────────────────────────────────────
export default function RecipeManagement({ embedded = false, storeMode = false, autoOpen = false }: { embedded?: boolean; storeMode?: boolean; autoOpen?: boolean } = {}) {
  const [search, setSearch]   = useState('');
  ...
  useEffect(() => { if (autoOpen) { setQuickAddOpen(true); setStoreSection('recipes'); } }, [autoOpen]);
