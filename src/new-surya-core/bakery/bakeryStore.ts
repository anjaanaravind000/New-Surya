...
interface BakeryState {
  ...
  // preventing the MaterialsProcurementModule list from unmounting and resetting card state.
...
// (e.g. ProductionOperationsModule's Completed-orders report, OrderingReceivingModule's
...
export async function fetchBakeryOrdersInRange(options: {
  ...
  // the semantics of orderCompletedAt() used by ProductionOperationsModule's Completed tab.
...
export const useBakeryStore = create<BakeryState>((set, get) => ({
  ...
  // don't cause the MaterialsProcurementModule list to flash/unmount and lose local state.
  //
  // EGRESS FIX: bounded to a 60-day rolling window, matching the same pattern
  // already used for the retail orders table (orderStore.ts). Every downstream
  // consumer (MaterialsProcurementModule, PackingDispatchModule, ProductionOperationsModule, OrderingReceivingModule,
  ...
  sendToProduction: async (orderId, selections, requestId) => {
    ...
    if (order?.targetBranch === 'SECONDARY_OUTLET') {
      const { useAuthStore } = await import('@/stores/authStore');
      const { useBranchOpsStore } = await import('@/branch/branchOpsStore');
      const user = useAuthStore.getState().currentUser;
      const acceptedBy = user?.displayName || user?.username || 'Store';
      ...
      useBranchOpsStore.getState().addNotification({
        ...
        title: 'Store accepted secondary outlet advance order',
  ...
  submitDispatch: async (orderId, entry) => {
    ...
    // Keep the Wholesale customer-order record behind the bakery workflow. It becomes
    // visible in "Received From Packing" only after Packing has dispatched it.
    const wholesaleOrderMatch = String(freshOrder.notes ?? '').match(/WHOLESALE_OUTLET_ORDER_ID:([^|]+)/);
    if (entry.branch === 'Wholesale' && wholesaleOrderMatch?.[1]) {
      const wholesaleOrderId = wholesaleOrderMatch[1];
      const itemDispatchTotal = updatedLog
        .filter(d => d.branch === 'Wholesale' && d.itemName === entry.itemName)
        .reduce((sum, d) => sum + Number(d.quantity || 0), 0);
      const { error: wholesaleItemError } = await supabase
        .from('wholesale_order_items')
        .update({ dispatched_quantity: itemDispatchTotal })
        .eq('order_id', wholesaleOrderId)
        .eq('item_name', entry.itemName);
      if (wholesaleItemError) throw new Error(`Wholesale dispatch sync failed: ${wholesaleItemError.message}`);

      const { error: wholesaleOrderError } = await supabase
        .from('wholesale_orders')
        .update({ status: allFullyDispatched ? 'dispatched' : 'pending_packing' })
        .eq('id', wholesaleOrderId);
      if (wholesaleOrderError) throw new Error(`Wholesale order status sync failed: ${wholesaleOrderError.message}`);
    }
