...
import { useOutletManagementReports } from "@/hooks/useOutletManagementReports";
import { PurchaseInvoicesTab } from "@/modules/incharge/PrimaryOutletManagementModule";
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden">
      <div className={cn(panelClass, "flex shrink-0 items-center justify-between gap-3 p-3")}>
        <div>
          ...
          <h2 className="font-display text-lg font-black">Materials → Production → Packing → Dispatch</h2>
...
export function PrimaryOutletPurchaseInvoicePanel() {
  const user = useAuthStore((state) => state.currentUser);
  const fetchBranchData = useBranchStore((state) => state.fetchBranchData);
  const [notice, setNotice] = useState("");
  const today = todayInput();
  const fromDate = `${today.slice(0, 4)}-01-01`;
  const dbReports = useOutletManagementReports(fromDate, today);
  const actorName = `Ordering Desk - ${user?.displayName || user?.username || "Ordering & Receiving"}`;
        ...
        <div className={cn(panelClass, "flex flex-wrap items-center justify-between gap-3 p-3")}>
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">
              Complete shared purchase invoice workflow
            </p>
            <h2 className="font-display text-lg font-black">Purchase Invoice Management</h2>
            <p className="mt-1 text-[11px] font-semibold text-muted-foreground">
              Create, edit, re-sync and review the shared purchase ledger. All actions are recorded as Ordering & Receiving.
            </p>
          </div>
          <button
            type="button"
...
export function PrimaryOutletPurchaseReturnPanel() {
  const stock = useBranchStore((state) => state.stock.PRIMARY_OUTLET);
  const user = useAuthStore((state) => state.currentUser);
  const userName = user?.displayName || user?.username || "Ordering & Receiving";
  ...
  const loadBase = useCallback(async () => {
    ...
    const [invoiceResult, historyResult] = await Promise.all([
      supabase
        ...
        .limit(500),
      supabase.from("primary_outlet_purchase_returns").select("id,return_no,supplier_name,invoice_number,return_date,reason_type,settlement_type,total_amount,entered_by,status,created_at").order("created_at", { ascending: false }).limit(500),
    ]);
    setLoading(false);
    ...
    void (async () => {
      ...
      if (itemIds.length > 0) {
        const { data: returnedRows, error: returnedError } = await supabase
          .from("primary_outlet_purchase_return_items")
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const { data, error: saveError } = await supabase.rpc("create_primary_outlet_order_purchase_return_secure", {
      ...
      p_entered_by: `Ordering Desk - ${userName}`,
    });
    setSaving(false);
    if (saveError) return setError(saveError.message);
    const result = (data || {}) as Record<string, unknown>;
    setSuccess(`${String(result.returnNo || "Purchase return")} posted and shared with Outlet Management.`);
    ...
    await Promise.all([loadBase(), useBranchStore.getState().fetchBranchData("PRIMARY_OUTLET")]);
  };
...
export function PrimaryOutletStockMovementPanel({ mode }: { mode: StockMovementMode }) {
  const { items: catalogItems } = useOperationalBranchCatalog("PRIMARY_OUTLET");
  const stock = useBranchStore((state) => state.stock.PRIMARY_OUTLET);
  const user = useAuthStore((state) => state.currentUser);
  const userName = user?.displayName || user?.username || "Ordering & Receiving";
  ...
  const submit = async () => {
    ...
    const { error: saveError } = await supabase.rpc("record_branch_waste_batch_secure", {
      ...
      p_checklist: ["Quantity physically verified", `Entered by Ordering Desk - ${userName}`],
    });
    setSaving(false);
    if (saveError) return setError(saveError.message);
    setSuccess(`${lines.length} item${lines.length > 1 ? "s" : ""} posted as ${mode === "Trans Out" ? "Transfer Out" : mode} and shared with Outlet Management.`);
    setLines([]);
    setMeta((current) => ({ ...current, reason: "", confirmed: false }));
    await Promise.all([loadRows(), useBranchStore.getState().fetchBranchData("PRIMARY_OUTLET")]);
  };
    ...
    <div className="grid h-full min-h-0 gap-3 overflow-auto xl:grid-cols-[minmax(340px,0.85fr)_minmax(0,1.15fr)]">
      <section className={cn(panelClass, "p-3")}>
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-start gap-2 rounded-xl border border-border bg-slate-50 p-3 text-xs font-bold"><input type="checkbox" checked={meta.confirmed} onChange={(event) => setMeta({ ...meta, confirmed: event.target.checked })} className="mt-0.5" /><span>I confirm every item's quantity was physically checked and may be deducted from usable outlet stock.</span></label>
...
export function PrimaryOutletStockOperationsPanel() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      ...
      <div className="min-h-0 flex-1">
        <PrimaryOutletStockMovementPanel mode={mode} />
      </div>
    </div>
  );
}
...
export function PrimaryOutletAdvanceOrdersPanel() {
  const { items: catalogItems } = useOperationalBranchCatalog("PRIMARY_OUTLET");
  const user = useAuthStore((state) => state.currentUser);
  const userName = user?.displayName || user?.username || "Ordering & Receiving";
  const stock = useBranchStore((state) => state.stock.PRIMARY_OUTLET);
  const advanceOrders = useBranchStore((state) => state.advanceOrders.PRIMARY_OUTLET);
  const fetchBranchData = useBranchStore((state) => state.fetchBranchData);
  const first = catalogItems[0];
  ...
  useEffect(() => { void fetchBranchData("PRIMARY_OUTLET"); }, [fetchBranchData]);
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const { error: saveError } = await supabase.rpc("create_primary_outlet_order_advance_order_secure", { p_customer_name: form.customerName.trim(), p_items: lines.map((line) => ({ barcode: line.barcode, itemName: line.itemName, quantity: Number(line.quantity), sellUnit: line.sellUnit, price: line.price, lineTotal: Number(line.quantity) * line.price, isCustom: false })), p_subtotal: subtotal, p_advance_amount: advance, p_advance_method: form.advanceMethod, p_delivery_date: form.deliveryDate, p_notes: form.notes.trim(), p_entered_by: `Ordering Desk - ${userName}` });
    setSaving(false);
    if (saveError) return setError(saveError.message);
    setSuccess(`Advance order saved. ${money(advance)} will appear to Admin as “Ordering & Receiving collected”.`);
