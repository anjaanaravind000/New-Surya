...
type SourceBranch = 'PRIMARY_OUTLET' | 'SECONDARY_OUTLET';
type Unit = 'kg' | 'pcs';
type TransferIn = {
  id: string;
...
const emptyForm = { source: 'PRIMARY_OUTLET' as SourceBranch, reference: '', itemName: '', expected: '', received: '', unit: 'kg' as Unit, remarks: '' };
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-black">Receive stock from retail outlets</h3>
          <p className="text-xs text-muted-foreground">Every receipt is posted centrally, added to the stock movement ledger, and retained for administration and executive audit.</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[390px_minmax(0,1fr)]">
        <form onSubmit={submit} className="rounded-2xl border bg-card p-4 space-y-3">
          ...
          <label className="block text-xs font-bold">Source branch<select value={form.source} onChange={(e) => setForm((v) => ({ ...v, source: e.target.value as SourceBranch }))} className="mt-1 h-11 w-full rounded-xl border bg-background px-3"><option value="PRIMARY_OUTLET">Primary Outlet</option><option value="SECONDARY_OUTLET">Secondary Outlet</option></select></label>
