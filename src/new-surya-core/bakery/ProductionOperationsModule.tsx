// src/bakery/ProductionOperationsModule.tsx  (Redesigned)
...
function printCompletedOrder(order: BakeryOrder) {
  ...
  printWindow.document.write(`
    ...
    <body>
      ...
      <div class="footer">New Surya · Baker Completed Order</div>
    </body>
    </html>
  `);
  ...
  const printClosure = () => {
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <div class="footer">New Surya · Baker Daily Closure</div>
      </body>
      </html>
    `);
    printWindow.document.close();
                ...
                >
                  ...
                  <span className="col-span-2">
                    <span className={cn('text-[9px] font-bold px-2 py-1 rounded-full',
                      r.targetBranch === 'SECONDARY_OUTLET' ? 'bg-blue-100 text-blue-700' :
                      r.targetBranch === 'PRIMARY_OUTLET' ? 'bg-amber-100 text-amber-700' :
                      'bg-emerald-100 text-emerald-700')}
                    >
                      {r.targetBranch === 'PRIMARY_OUTLET' ? 'Primary Outlet' : r.targetBranch === 'SECONDARY_OUTLET' ? 'Secondary Outlet' : r.targetBranch === 'Wholesale' ? 'Wholesale Outlet' : r.targetBranch}
                    </span>
                  </span>
...
// ─── Main ─────────────────────────────────────────────────────────────────────
type ProductionOperationsModuleTab = 'orders' | 'corrections' | 'completed' | 'closure';
const BAKER_TABS: ProductionOperationsModuleTab[] = ['orders', 'corrections', 'completed', 'closure'];

export default function ProductionOperationsModule({
  destination = 'baker',
  title,
}: {
  ...
  const requestedTab = searchParams.get('tab') as ProductionOperationsModuleTab | null;
  const tab: ProductionOperationsModuleTab = requestedTab && BAKER_TABS.includes(requestedTab) ? requestedTab : 'orders';
