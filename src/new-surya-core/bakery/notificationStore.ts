...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  pushPackingDiscrepancy: async (orderId, orderNumber, branch, items) => {
    ...
    // Previously only an `admin` row was created, while the PRIMARY_OUTLET/Secondary Ordering & Receiving
    // dashboards load notifications using their own roles (`stock_audit_primary` /
    // `stock_audit_secondary`). As a result, branch users never received the alert.
    const normalizedBranch = branch.trim().toUpperCase();
    const receiverRole = normalizedBranch === 'PRIMARY_OUTLET'
      ? 'stock_audit_primary'
      : normalizedBranch === 'SECONDARY_OUTLET'
