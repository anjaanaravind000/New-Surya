...
// Utilities for:
//   1. Parsing per-unit weights from SECONDARY_OUTLET item names   e.g. "Banana chips (200g)" → 200g
//   2. Converting receiver-entered pcs quantities → kg   e.g. 5 pcs × 200g = 1.000 kg
//   3. Matching SECONDARY_OUTLET / PRIMARY_OUTLET item names to RECIPE_DEFINITIONS keys
 ...
 * SECONDARY_OUTLET cookies are ordered as 200 g packets, while their production recipes
...
export function resolveItemWeightGrams(itemId: string, itemName: string): number | null {
  ...
  const secondary_outletBarcode = itemId.toLowerCase().match(/^secondary_outlet-(\d+)$/)?.[1];
  const barcode = secondary_outletBarcode ? Number(secondary_outletBarcode) : 0;
  if (barcode >= 2090 && barcode <= 2108) return 200;
  if (secondary_outletBarcode && SECONDARY_OUTLET_ITEMS.some(item =>
    item.barcode === barcode && item.category === 'COOKIES'
  )) return 200;
  if (secondary_outletBarcode && BAKERY_ITEMS.some(item =>
 ...
 * to a name-based slug match (works for SECONDARY_OUTLET / PRIMARY_OUTLET items with barcode-derived IDs).
 */
