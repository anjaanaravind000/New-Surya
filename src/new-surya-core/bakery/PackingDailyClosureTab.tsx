...
export default function PackingDailyClosureTab({ onCounterStatusChange }: { onCounterStatusChange?: (isOpen: boolean) => void }) {
  ...
  const branchCreditPayments = useBranchStore(state => state.creditPayments.PRIMARY_OUTLET);
  ...
  useEffect(() => {
    void Promise.all([fetchOrders(true), fetchBranchData('PRIMARY_OUTLET')]).finally(() => void loadClosure());
  ...
  const branchSummary = useMemo(() => {
    const result: Record<Branch, { kg: number; pcs: number; orders: number; ids: Set<string> }> = {
      SECONDARY_OUTLET: { kg: 0, pcs: 0, orders: 0, ids: new Set() },
      PRIMARY_OUTLET: { kg: 0, pcs: 0, orders: 0, ids: new Set() },
      Wholesale: { kg: 0, pcs: 0, orders: 0, ids: new Set() },
    };
  ...
  const refresh = async () => {
    ...
    await Promise.all([fetchOrders(true), fetchBranchData('PRIMARY_OUTLET')]);
    await loadClosure();
  };
