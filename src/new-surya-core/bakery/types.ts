...
export type BakeryRole = 'stock_audit_secondary' | 'stock_audit_primary' | 'store' | 'baker' | 'sweet_master' | 'savouries_master' | 'cookies_master' | 'puffs_master' | 'bakery_master' | 'packing';
...
export interface BakeryOrderItem {
  itemId: string;
  itemName: string;
  quantity: number;       // always in kg (for SECONDARY_OUTLET Nos items, already converted) or natural unit
  isCustom?: boolean;
  /** Optional custom/cake reference image sent by the branch. Stored inside items JSONB. */
  attachmentName?: string;
  attachmentDataUrl?: string;
  /** SECONDARY_OUTLET Nos items: the raw pcs count the receiver entered before conversion */
  originalPcs?: number;
  /** SECONDARY_OUTLET Nos items: per-unit weight in grams used for the pcs→kg conversion */
  weightGrams?: number;
  /**
...
export type Branch = 'SECONDARY_OUTLET' | 'PRIMARY_OUTLET' | 'Wholesale';
...
export const BAKERY_ITEMS: { id: string; name: string; icon: string; category: string }[] = [
  ...
  { id: 'primary_outlet-special-mixture',          name: 'New Surya Special Mixture',             icon: '🥜', category: 'Savouries' },
...
export const BRANCHES: Branch[] = ['SECONDARY_OUTLET', 'PRIMARY_OUTLET', 'Wholesale'];
