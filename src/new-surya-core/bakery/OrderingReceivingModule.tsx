// ─── OrderingReceivingModule — Branch-locked, role-driven ─────────────────────
// stock_audit_secondary → SECONDARY_OUTLET branch only, Secondary Outlet items only
// stock_audit_primary   → PRIMARY_OUTLET branch only,   Outlet items only
//
...
import {
  ...
  PrimaryOutletPurchaseInvoicePanel,
  PrimaryOutletPurchaseReturnPanel,
  PrimaryOutletStockOperationsPanel,
} from "./ReceiverSharedTabs";
...
const BRANCH_META: Record<string, BranchMeta> = {
  stock_audit_secondary: {
    branch: "SECONDARY_OUTLET",
    label: "Secondary Ordering & Receiving",
    icon: "🏙️",
    dot: "bg-blue-500",
    ring: "ring-blue-400",
...
function mapWholesaleShop(row: Record<string, unknown>): WholesaleShop {
  return {
    id: row.id as string,
...
function mapWholesalePrice(row: Record<string, unknown>): WholesaleShopPrice {
  return {
    id: row.id as string,
...
function mapWholesaleOrder(row: Record<string, unknown>): WholesaleOrder {
  return {
    id: row.id as string,
...
function mapWholesaleOrderItem(row: Record<string, unknown>): WholesaleOrderItem {
  return {
    id: row.id as string,
    ...
    try {
      const [invoiceRes, returnRes, wasteRes, operationRes] = await Promise.all([
        supabase
          ...
          .limit(500),
        supabase
          .from("primary_outlet_purchase_returns")
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden">
      <div className="shrink-0 rounded-2xl border border-border bg-white p-3 shadow-sm">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Shared from Outlet Management</p>
...
function PlacedOrdersPanel({
  ...
  const handleRemoveItem = async () => {
    ...
    const tableName = branch === "Wholesale" ? "wholesale_orders" : "bakery_orders";
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="h-11 rounded-2xl surya-gradient px-4 text-sm font-body font-black text-primary-foreground flex items-center justify-center gap-2 disabled:opacity-45 active:scale-95 transition-all"
            >
...
function WholesaleOrderPanel({
  ...
  const [cart, setCart] = useState<Record<string, WholesaleDraftItem>>({});
  ...
  const shopItems = useMemo(() => {
    ...
    return prices
      ...
      .map((price) => {
        const master = primary_outletItems.find((item) => normalize(item.name) === normalize(price.itemName));
        return {
          ...price,
  ...
  }, [prices, selectedShop, primary_outletItems]);
  ...
  const saveOrder = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to send wholesale order.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <section className="rounded-3xl border border-border bg-card p-4 shadow-soft space-y-4">
        ...
        {activeShops.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-muted/30 p-10 text-center">
            ...
            <p className="font-body text-sm font-black text-foreground">No active wholesale customers found.</p>
            <p className="mt-1 text-xs font-bold text-muted-foreground">Add shops and shop-wise prices from the wholesale outlet dashboard first.</p>
          </div>
...
function WholesaleHistoryPanel({ orders, orderItems, loading }: { orders: WholesaleOrder[]; orderItems: WholesaleOrderItem[]; loading: boolean }) {
  const [preset, setPreset] = useState<DatePreset>("today");
  const [customStart, setCustomStart] = useState(toDateInputValue(new Date()));
  const [customEnd, setCustomEnd] = useState(toDateInputValue(new Date()));
  ...
  const downloadExcelReport = () => {
    ...
    const rows: Array<Array<string | number | undefined>> = [
      ["Wholesale Customer Ordered Quantity Report"],
      ["Date Range", `${toDateInputValue(range.start)} to ${toDateInputValue(range.end)}`],
      [],
      ["Item Summary"],
    ...
    downloadExcel(`wholesale-ordered-quantity-${toDateInputValue(range.start)}-to-${toDateInputValue(range.end)}.xls`, rows);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-3xl border border-border bg-card p-4 shadow-soft">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[11px] font-body font-black uppercase tracking-[0.18em] text-muted-foreground">Wholesale customer order history</p>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-dashed border-border bg-white/70 p-10 text-center">
          <ClipboardList className="mx-auto mb-3 size-10 text-muted-foreground" />
          <p className="font-body text-sm font-black text-foreground">No wholesale orders in this range.</p>
        </div>
      ) : (
...
function WholesaleAlertsPanel({ orders, orderItems }: { orders: WholesaleOrder[]; orderItems: WholesaleOrderItem[] }) {
    ...
    <div className="space-y-3">
      <div className="rounded-3xl border border-border bg-white/90 p-4 shadow-soft">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-body font-black text-foreground">Wholesale Packing Alerts</p>
        ...
        <article key={item.id} className="rounded-3xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="text-xs font-bold text-muted-foreground">{order?.shopName ?? "Wholesale customer"} · {order?.orderNumber ?? item.orderId}</p>
            </div>
      ...
      <div className="w-full max-w-sm overflow-hidden rounded-3xl border border-white/60 bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-600">Outlet Physical Stock</p>
...
function StockCountPanel({
  ...
  branch: Extract<Branch, "PRIMARY_OUTLET" | "SECONDARY_OUTLET">;
    ...
    setCounts((prev) => {
      ...
      rows.forEach((row) => {
        if (!touchedCounts.current[row.itemName]) {
          next[row.itemName] = branch === "PRIMARY_OUTLET" ? "0" : String(row.systemQty);
        }
      });
      return next;
    });
  ...
  const differenceCount = rows.filter((row) => {
    if (branch === "PRIMARY_OUTLET" && !countedItems[row.itemName]) return false;
    const physical = Number(counts[row.itemName] || 0);
    return Math.abs(row.systemQty - physical) > 0.0001;
  }).length;

  const submit = async () => {
    if (branch === "PRIMARY_OUTLET") {
      const uncountedRows = rows.filter((row) => !countedItems[row.itemName]);
      if (uncountedRows.length > 0) {
        setNoticeTone("error");
        setNotice(
          `${uncountedRows.length} item${uncountedRows.length === 1 ? " is" : "s are"} still uncounted. Count each item or explicitly confirm zero before sending to Outlet Management.`,
        );
        return;
      }
    }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[520px]">
            ...
            <div className="max-h-[65vh] overflow-y-auto divide-y divide-border/60">
              {rows.map((row) => {
                const hasConfirmedCount = branch !== "PRIMARY_OUTLET" || Boolean(countedItems[row.itemName]);
                const physical = Number(counts[row.itemName] || 0);
      ...
      {branch === "PRIMARY_OUTLET" && calculatorRow && (
...
export default function OrderingReceivingModule({ branchOverride }: { branchOverride?: Branch } = {}) {
  ...
  const [wholesaleShops, setWholesaleShops] = useState<WholesaleShop[]>([]);
  const [wholesalePrices, setWholesalePrices] = useState<WholesaleShopPrice[]>([]);
  const [wholesaleOrders, setWholesaleOrders] = useState<WholesaleOrder[]>([]);
  const [wholesaleOrderItems, setWholesaleOrderItems] = useState<WholesaleOrderItem[]>([]);
  const [wholesaleLoading, setWholesaleLoading] = useState(false);

  // Derive branch from role
  const role = currentUser?.role as UserRole | undefined;
  const roleMeta = role ? BRANCH_META[role] : undefined;
  const branch = branchOverride ?? (roleMeta?.branch as Branch | undefined) ?? "PRIMARY_OUTLET";
  const meta = roleMeta ?? Object.values(BRANCH_META).find(item => item.branch === branch) ?? BRANCH_META.stock_audit_primary;
  const requestedTab = tabFromParams(searchParams.get("tab"));
  const primary_outletOnlyTabs: TabKey[] = ["po", "purchase-invoice", "purchase-return", "stock-movements", "advance", "closure"];
  const tab = branch !== "PRIMARY_OUTLET" && primary_outletOnlyTabs.includes(requestedTab) ? "order" : requestedTab;
  const userName =
    currentUser?.displayName || currentUser?.username || `${branch ?? "PRIMARY_OUTLET"} Receiver`;
  ...
  const loadWholesaleReceiverData = useCallback(async () => {
    if (branch !== "Wholesale") return;
    setWholesaleLoading(true);
    try {
      const [shopsRes, pricesRes, ordersRes, itemsRes] = await Promise.all([
        supabase.from("wholesale_shops").select("id, shop_name, whatsapp_number, address, is_active").eq("is_active", true).order("shop_name", { ascending: true }),
        supabase.from("wholesale_shop_price_lists").select("id, shop_id, item_name, item_unit, unit_price, is_active").eq("is_active", true),
        supabase.from("wholesale_orders").select("id, order_number, shop_id, shop_name, status, subtotal, created_by, created_at, notes").order("created_at", { ascending: false }).limit(250),
        supabase.from("wholesale_order_items").select("id, order_id, item_name, unit, quantity, unit_price, line_total, dispatched_quantity, received_quantity").order("created_at", { ascending: true }).limit(3000),
      ]);
      if (shopsRes.error) throw shopsRes.error;
      if (pricesRes.error) throw pricesRes.error;
      if (ordersRes.error) throw ordersRes.error;
      if (itemsRes.error) throw itemsRes.error;
      setWholesaleShops((shopsRes.data ?? []).map(mapWholesaleShop));
      setWholesalePrices((pricesRes.data ?? []).map(mapWholesalePrice));
      setWholesaleOrders((ordersRes.data ?? []).map(mapWholesaleOrder));
      setWholesaleOrderItems((itemsRes.data ?? []).map(mapWholesaleOrderItem));
    } finally {
  ...
  useEffect(() => {
    loadWholesaleReceiverData();
  }, [loadWholesaleReceiverData, refreshKey]);

  useEffect(() => {
    if (branch !== "PRIMARY_OUTLET" && branch !== "SECONDARY_OUTLET") return;
        ...
        : tab === "notifications"
          ...
          : tab === "stock"
            ...
            : tab === "po"
              ...
              : tab === "purchase-invoice"
                ? "Create purchase invoices and view the same shared records available to Outlet Management."
                : tab === "purchase-return"
                  ...
                  : tab === "stock-movements"
                    ...
                    : tab === "stock-count"
                      ...
                      : tab === "advance"
                        ? "Open the Ordering & Receiving counter first, then take advance orders and collect payments."
                        : tab === "closure"
                          ? "Open the counter, verify Ordering & Receiving advance collections and save the same cashier closure used by Retail Outlet."
  ...
  return (
    <div className="order-receiver-workspace dashboard-screen flex h-full min-h-0 flex-col overflow-hidden bg-transparent pb-2 font-semibold">
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        ...
        {tab === "placed" && (
          branch === "Wholesale" ? (
            <WholesaleHistoryPanel orders={wholesaleOrders} orderItems={wholesaleOrderItems} loading={wholesaleLoading} />
          ) : (
            <PlacedOrdersPanel
              branch={branch}
            />
        ...
        {tab === "notifications" && (
          branch === "Wholesale" ? (
            <WholesaleAlertsPanel orders={wholesaleOrders} orderItems={wholesaleOrderItems} />
          ) : (
          <div className="space-y-3">
            {/* Header row */}
        ...
        {tab === "stock" && (branch === "PRIMARY_OUTLET" || branch === "SECONDARY_OUTLET") && (
          <StockTab
            branch={branch}
        ...
        {tab === "po" && branch === "PRIMARY_OUTLET" && <PurchaseOrderTab branchScope="PRIMARY_OUTLET" />}

        {tab === "purchase-invoice" && branch === "PRIMARY_OUTLET" && <PrimaryOutletPurchaseInvoicePanel />}

        {tab === "purchase-return" && branch === "PRIMARY_OUTLET" && <PrimaryOutletPurchaseReturnPanel />}

        {tab === "stock-movements" && branch === "PRIMARY_OUTLET" && <PrimaryOutletStockOperationsPanel />}

        {tab === "advance" && branch === "PRIMARY_OUTLET" && (
          <AdvanceCakeOrdersTab branch="PRIMARY_OUTLET" branchStock={stock.PRIMARY_OUTLET} source="primary_outlet-order" />
        )}

        {tab === "closure" && branch === "PRIMARY_OUTLET" && (
          <CashierClosureTab branch="PRIMARY_OUTLET" branchStock={stock.PRIMARY_OUTLET} source="primary_outlet-order" />
        )}

        {tab === "stock-count" && (branch === "PRIMARY_OUTLET" || branch === "SECONDARY_OUTLET") && (
