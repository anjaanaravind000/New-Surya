...
export type CatalogBranch = 'PRIMARY_OUTLET' | 'SECONDARY_OUTLET';
...
const seedItems = (branch: CatalogBranch): BranchCatalogItem[] =>
  (branch === 'PRIMARY_OUTLET' ? PRIMARY_OUTLET_ITEMS : SECONDARY_OUTLET_ITEMS).map((item) => ({
    branch,
...
export const useBranchCatalogStore = create<BranchCatalogState>((set, get) => ({
  items: { PRIMARY_OUTLET: seedItems('PRIMARY_OUTLET'), SECONDARY_OUTLET: seedItems('SECONDARY_OUTLET') },
  loaded: { PRIMARY_OUTLET: false, SECONDARY_OUTLET: false },
  loading: { PRIMARY_OUTLET: false, SECONDARY_OUTLET: false },
  errors: { PRIMARY_OUTLET: null, SECONDARY_OUTLET: null },
  ...
  addItem: async (branch, item, updatedBy) => {
    ...
    if (!rpcResult.error && rpcResult.data) {
      ...
      const rangeStart = branch === 'PRIMARY_OUTLET' ? 1000 : 2000;
