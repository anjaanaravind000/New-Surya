...
export const useMenuStore = create<MenuState>()((set, get) => ({
  ...
  updateItem: async (id: string, updates: Partial<MenuItem>) => {
    ...
    if (priceChanged || nameChanged) {
      ...
      // Secondary Outlet Management or Outlet Management changing retail items -> notify 'admin' (super admin must see it)
      // Super Admin changing retail items -> notify 'admin' (self-audit, appears in own feed)
      const changerRole = useAuthStore.getState().currentUser?.role ?? 'admin';
      const recipientRole = (changerRole === 'branch_incharge_secondary' || changerRole === 'branch_incharge_primary') ? 'admin' : 'admin';

      const { error: notifError } = await supabase.from('admin_notifications').insert({
        type:           'price_change',
        title:          `Retail Menu Updated - ${effectiveName}`,
        body:           `${changes.join(' | ')} | Retail menu`,
        ref_label:      `Retail | Item ID ${id}`,
        meta:           {
          branch:   'RETAIL',
          itemId:   id,
          name:     effectiveName,
