        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            {label: "5. Forecast to Production Plan", desc: "Convert forecast need into a kitchen approval request", action: () => {
              ...
              dispatch({ type:'create-production', productId: row.id, requestedQty: row.forecastNeed, requestedBy:'Executive Forecast', notes:'Client-demo forecast plan', branchDemand:{ 'marathahalli': Math.round(row.forecastNeed * .4), 'sarjapur-road': Math.round(row.forecastNeed * .3), 'kadubeesanahalli': Math.round(row.forecastNeed * .2), 'koramangala': Math.round(row.forecastNeed * .1) } });
