...
export default function AdminDashboard() {
  ...
  return <Shell title="New Surya Administration" subtitle="Live performance, approvals and operational health across every branch and production unit.">
    ...
    {tab === 'Users & Access' && <div className="grid min-w-0 gap-5 2xl:grid-cols-[360px_minmax(0,1fr)]">
      <Card title="Create branch user" description="Create one secure login, choose its role, and assign it to your operating branch. Add more branches from Branch Setup as you grow.">
    ...
    {tab === 'Purchase/GRN' && <div className="space-y-5">
      ...
      <Card title="Purchase orders" description="Low stock can create PO, and GRN receipt increases raw material inventory." action={<ActionButton tone="green" onClick={() => dispatch({ type:'create-purchase-order', po:{ supplierId:'sup-grocery', expectedDate:new Date(Date.now()+48*3600_000).toISOString().slice(0,10), status:'draft', createdBy:'New Surya Executive', lines:[{ ingredientId:'ing-jaggery', qty:120, rate:118 }] } })}>Create sample PO</ActionButton>}>
    ...
    {tab === 'Promotions & Loyalty' && <div className="space-y-5">
      ...
      <Card title="Promotions & Campaign Studio" description="Create rules that automatically apply at billing (BOGO, % off, points multipliers, happy hours, birthday offers). Central administration controls these settings.">
    ...
    {tab === 'Demand Forecasting & MRP' && <div className="space-y-5">
      <Card title="Predictive Demand Forecasting & MRP (Material Requirements Planning)" description="Sales history + simple intelligent forecast → suggested production quantities per product/branch. This is a key differentiator vs Petpooja/GOFRUGAL — reduces over/under production dramatically.">
        ...
        <DataTable rows={forecastRows} columns={[{key:'name',label:'Product'},{key:'category',label:'Category'},{key:'price',label:'Price',render:p => money(p.price)},{key:'thirtyDaySales',label:'30-day Sales'},{key:'forecastNeed',label:'Forecasted Need (next 7 days)',render:p => <b className="font-ticket text-lg">{p.forecastNeed} {p.unit}</b>},{key:'id',label:'Suggested Action',render:p => <ActionButton tone="green" onClick={() => { dispatch({ type:'create-production', productId: p.id, requestedQty: p.forecastNeed, requestedBy:'Executive Forecast', notes:'Forecast-generated MRP plan', branchDemand: { 'marathahalli': p.forecastNeed } }); notify(`${p.name} forecast converted into a kitchen production request.`); }}>Create Production Plan from Forecast</ActionButton>}]} />
      </Card>
    ...
    {tab === 'Compliance & GST' && <div className="space-y-5">
      <Card title="GST / e-Invoice / GSTR Export Center" description="Full compliance support beyond basic Tally export. Central administration controls data export formats.">
    ...
    {tab === 'Notifications Hub' && <Card title="Notifications, Alerts & Communication Center" description="Central control for in-app, WhatsApp, SMS, Email alerts. Central administration sets the rules.">
      ...
      <Card title="Interactive Full System Demo" description="Simulate the complete bakery operation across Admin, Kitchen and Branch Billing. These buttons are prepared for a client walkthrough.">
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            {label: "2. Production Approval Gate", desc: "Approve kitchen plan and deduct raw materials only after executive approval", action: () => {
              const plan = state.productionPlans.find(p => p.status === 'pending-admin-approval');
              if (plan) {
                dispatch({ type:'approve-production', planId: plan.id, adminName:'New Surya Executive Demo' });
            ...
            {label: "3. Central Dispatch + Branch Receive", desc: "Create dispatch with route, vehicle, crate and branch receive flow", action: () => {
              dispatch({ type:'create-dispatch', toBranchId:'marathahalli', route:'East Route 2', driver:'Demo Driver', vehicleNo:'KA-01-DEMO', crates:['CR-DEMO-1'], lines:[{ productId:'prod-mixture', qty:20, batchNo:'MX-0706-A' }] });
            ...
            {label: "5. Forecast to Production Plan", desc: "Convert forecast need into a kitchen approval request", action: () => {
              ...
              dispatch({ type:'create-production', productId: row.id, requestedQty: row.forecastNeed, requestedBy:'Executive Forecast', notes:'Client-demo forecast plan', branchDemand:{ 'marathahalli': row.forecastNeed } });
          ...
          <div>
            ...
            <div className="mt-3 p-3 border border-dashed border-ink/20 rounded text-sm">
              ...
              <ActionButton tone="green" onClick={() => { 
                dispatch({ type:'receive-purchase-order', poId: 'po-001', invoiceNo: 'INV-2026-0742', receivedBy: 'Executive' }); 
    ...
    {(['Business Management','Executive Control','Menu Management','Sales Reports','Staff Management','Attendance & Payroll','Order History','Invoice Review','Alerts & Notifications','Digital Menu Management','Product Master','Recipe Management'] as readonly string[]).includes(tab) && <AdminIntegratedFeature module={tab as AdminIntegratedModule} />}
