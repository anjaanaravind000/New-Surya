...
import { StockAuditIntegratedFeature } from '../components/IntegratedFeatureModules';

const existingTabs = ['Audit Desk', 'Physical Count', 'Incoming Verification', 'Purchase Match', 'Waste & Movements', 'History', 'Ordering & Receiving', 'Extended Ordering & Receiving', 'Complete Feature Centre'] as const;
...
export default function StockAuditDashboard() {
  ...
  const [tab, setTab] = useState<Tab>(() => searchParams.get('suite') === 'complete-feature-centre' ? 'Complete Feature Centre' : searchParams.get('suite') === 'secondary-ordering-receiving' || appRole === 'stock_audit_secondary' ? 'Extended Ordering & Receiving' : searchParams.get('suite') === 'ordering-receiving' || appRole === 'stock_audit_primary' ? 'Ordering & Receiving' : 'Audit Desk');
  ...
  return <Shell title="Stock Audit" subtitle="Independent physical counting, inward checks, invoice matching, variance evidence and approval-ready history for your branch.">
    ...
    {tab === 'Complete Feature Centre' && <CompleteFeatureCenter dashboard="stock-audit" initialModule={searchParams.get('module') ?? undefined} />}
    {tab === 'Ordering & Receiving' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" />}
    {tab === 'Extended Ordering & Receiving' && <StockAuditIntegratedFeature module="Secondary Ordering & Receiving" />}
    {tab === 'Place Order' && <StockAuditIntegratedFeature module={appRole === 'stock_audit_secondary' ? 'Secondary Ordering & Receiving' : 'Primary Ordering & Receiving'} internalTab="order" />}
    {tab === 'Live Order Status' && <StockAuditIntegratedFeature module={appRole === 'stock_audit_secondary' ? 'Secondary Ordering & Receiving' : 'Primary Ordering & Receiving'} internalTab="live" />}
    {tab === 'Placed Orders' && <StockAuditIntegratedFeature module={appRole === 'stock_audit_secondary' ? 'Secondary Ordering & Receiving' : 'Primary Ordering & Receiving'} internalTab="placed" />}
    {tab === 'Packing Alerts' && <StockAuditIntegratedFeature module={appRole === 'stock_audit_secondary' ? 'Secondary Ordering & Receiving' : 'Primary Ordering & Receiving'} internalTab="notifications" />}
    {tab === 'Purchase Order' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="po" />}
    {tab === 'Purchase Invoice' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="purchase-invoice" />}
    {tab === 'Purchase Return' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="purchase-return" />}
    {tab === 'Stock Movements' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="stock-movements" />}
    {tab === 'Daily Stock Take' && <StockAuditIntegratedFeature module={appRole === 'stock_audit_secondary' ? 'Secondary Ordering & Receiving' : 'Primary Ordering & Receiving'} internalTab="stock-count" />}
    {tab === 'Advance Orders' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="advance" />}
    {tab === 'Order Closure' && <StockAuditIntegratedFeature module="Primary Ordering & Receiving" internalTab="closure" />}
    {!(['Place Order','Live Order Status','Placed Orders','Packing Alerts','Purchase Order','Purchase Invoice','Purchase Return','Stock Movements','Daily Stock Take','Advance Orders','Order Closure'] as readonly string[]).includes(tab) && isExtensionTab('stock-audit', tab) && <OperationalWorkbench scope="stock-audit" module={tab} branchName={state.branches.find(branch => branch.id === state.selectedBranchId)?.name} />}
  </Shell>;
}
