...
const existingTabs = ['Today', 'Orders', 'Inventory', 'Team', 'Cash & Closure', 'Approvals', 'Reports'] as const;
...
export default function BranchInchargeDashboard() {
  ...
  return <Shell title="Branch Control" subtitle={`${currentBranch?.name ?? 'Selected branch'} daily operations, people, stock, orders, approvals and cash control`}>
    ...
    {isExtensionTab('branch-incharge', tab) && <OperationalWorkbench scope="branch-incharge" module={tab} branchName={currentBranch?.name} />}
  </Shell>;
}
