...
const existingTabs = ['POS Billing', 'Counter', 'Online Orders', 'Advance Orders', 'Credit', 'Goods Receipt', 'Stock', 'Returns', 'Daily Closure', 'Reports', 'Devices', 'Debug'] as const;
...
export default function BranchBillingDashboard() {
  ...
  return <Shell title="Branch POS" subtitle={`${currentBranch?.name ?? 'Selected branch'} counter billing, online orders, stock receiving, returns and closure`}>
    ...
    {isExtensionTab('branch', tab) && <OperationalWorkbench scope="branch" module={tab} branchName={currentBranch?.name} />}
  </Shell>;
}
